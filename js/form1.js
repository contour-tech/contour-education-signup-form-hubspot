...
var ContourForm1Logic = function () {
  ...
  var FIELD_SELECTORS = {
    ...
    // The four dedicated guardian boxes behind the "Add" prompt on the
    // Student flow. Distinct from guardianPhone above, which is the visitor's
    // OWN phone box and only becomes the guardian's on the Guardian flow.
    addGuardianFirstName: '[name="guardian_s_first_name"]',
    addGuardianLastName: '[name="guardian_s_last_name"]',
    addGuardianEmail: '[name="guardian_email"]',
    addGuardianPhone: '[name="guardian_phone"]',
    noProgramWaitlist: '[name="join_no_program_waitlist"]',
    referral: '[name="referral"]',
    referralCodeEntered: '[name="referral_code_entered"]',
    contactMethod: '[name="how_did_they_contact_us"]',
    signedUpBy: '[name="signed_up_by"]',
    signedUpByOwnerId: '[name="signed_up_by_owner_id"]'
  };
  ...
  var FEATURE_DEFAULTS = {
    ...
    //     separator rule beside it goes solid navy to match. Tried as the
    //     default and taken back the same day — the outline reads quieter
    //     against the field labels (Amrit, 4 Sep 2026). Implies the pill,
    //     so it stands alone or on top of outline.
    ...
    // Raises the page's 768px cap on the form to 900px, so the widest campus
    ...
    // When the visitor has picked one program and another is also available
    // for their location, year level and intake, a small suggestion box under
    // the cards names the one they have not picked, with a "Show subjects"
    // button that ticks it. Interstate families were picking UCAT alone and
    // never learning the tutoring existed (Angad, 20 Sep 2026). A suggestion,
    // not a default: program_interest keeps meaning what the visitor chose,
    // so the coverage check and the routing built on it stay as they are.
    programNudge: true,
    // Ticks every program the visitor is eligible for the moment location,
    // year level and intake are in, so the whole offering is on screen
    // without a click. Stops the moment they touch a card themselves: an
    // untick stays unticked through later matrix changes, and the nudge row
    // above offers the brand back. The coverage check still holds — a ticked
    // program with no subject is an error that sends them back to the cards
    // to untick it (team decision, 21 Sep 2026). Supersedes
    // autoSelectSingleProgram.
    autoSelectAllPrograms: true,
    ...
    // A referral code is something a NEW visitor is given by a friend. Someone
    // arriving on their own pre-fill link has already signed up once, so the
    // box is an invitation to claim a discount they cannot have, and leaving
    // it on screen is the only reason anyone would try.
    hideReferralCodeOnPrefill: true,
    // The "Add a parent or guardian" prompt on the Student flow, and the four
    // guardian_* boxes it opens. They are HubSpot dependents of
    // web_form_contact_type = Student, so they only exist in the DOM on that
    // flow; this file moves them out of the dependent host — where HubSpot
    // renders them ABOVE the visitor's own rows — into their own block under
    // the Contact Information box, and keeps them shut until the prompt is
    // pressed. Off leaves the four boxes where HubSpot put them, which is why
    // the flag exists: a page whose form has not been published with the
    // fields yet is unaffected either way, and one that has can turn the
    // block off without a deploy (Amrit, 22 Sep 2026).
    addGuardianOnStudentFlow: true,
    ...
    // The gate still folds when `sectionUnlocked.study` goes true, and its
    // header still folds/reopens on press either way — this only switches
    // off the early, touch-triggered fold.
    intakeYearAutoFoldOnContact: false,
    // Whether a student-email check that could NOT be resolved holds the
    // submit. Off: an address we failed to look up goes through, the way it
    // did when the check was built.
    //
    // The lookup depends on a Cloud Function, HubSpot's search API and the
    // visitor's network. When any of those is having a bad minute the answer
    // is "unknown", and blocking on it means our outage stops every signup it
    // cannot resolve — losing the family silently, with no record left behind
    // for anyone to notice. A duplicate is cheap next to that: the backend
    // already dedupes, and manual review already exists for what slips past.
    // The panel still says we could not check; it just does not bar the door.
    blockOnUnresolvedEmailCheck: false
  };
  ...
  // VSE Foundation (VSE-FOEN/FOMA/FOWR, Year 6) and VSE Mastery
  // (VSE-MAEN/MAMA/MAWR, Year 8) were dropped from the 2026 VIC cells on
  // 25 Sep 2026: both were discontinued at the end of T2 2026, and VSE Core
  // is the only 2026 Selective Entry signup now (Wassim).
  ...
  function injectCampusMapStyles() {
    ...
    style.textContent =
      ...
      // at the form's width cap. Overlaying costs no width at any viewport,
  ...
  function enforcePrefillLinkPresentation() {
    ...
    // Safe to hide outright: the field is optional, so HubSpot has nothing to
    // validate on it. A required field hidden this way blocks the submit with
    // an error nobody can see — see the school placeholder.
    if (featureEnabled("hideReferralCodeOnPrefill")) {
      var referralCode = q(FIELD_SELECTORS.referralCodeEntered);
      if (referralCode) hideFieldWrapper(referralCode);
    }
  }
  /* =========================================================
  ...
  function updatePersonGroups() {
    ...
    if (!enabled) {
      ...
      // The guardian block is its own feature and does not ride on the person
      // grouping: turning the segments off must not strand four boxes above
      // the visitor's own rows.
      updateAddGuardian();
      return;
    }
    if (variant === "tabs") updatePersonTabs(guardian); else updatePersonStacked(guardian);
    // After the person segments, so the block's anchor — the last of the
    // visitor's own rows — is already in its final position for this pass.
    updateAddGuardian();
  }
  /* =========================================================
  ...
  function positionGuardianSegment(guardian, studentRows, guardianRows) {
    ...
    // With the guardian block on, the Student flow's destination is INSIDE
    // the dependent host rather than out of it: HubSpot renders the guardian's
    // four boxes above the visitor's own rows, and they are React's children
    // of that host and must stay its children (see ADD GUARDIAN DETAILS), so
    // the only way to put them last is to step the visitor's fieldsets in
    // ahead of them. The mirror of the Guardian branch above.
    var addGuardianRows = addGuardianEnabled() ? addGuardianWraps() : [];
    var host = addGuardianHost();
    if (host && addGuardianRows.length > 0 && addGuardianRows[0].parentElement === host) {
      var stop = addGuardianRows[0];
      var placed = guardianRows.every(function (fs) {
        return fs.parentElement === host && !!(fs.compareDocumentPosition(stop) & Node.DOCUMENT_POSITION_FOLLOWING);
      });
      if (placed) return;
      guardianRows.forEach(function (fs) {
        host.insertBefore(fs, stop);
      });
      return;
    }
    var contact = q(FIELD_SELECTORS.contactType);
    var wrap = contact ? fieldWrapper(contact) : null;
    var nativeHost = wrap ? wrap.parentElement : null;
    var outer = nativeHost && nativeHost.classList.contains("hs-dependent-field") ? nativeHost.parentElement : null;
  ...
  function updatePersonStacked(guardian) {
    ...
    if (guardian) {
      ...
      // The visitor's rows have stepped inside the dependent host to sit above
      // the guardian block, so the host is the box's container here the same
      // way it is on the Guardian flow, and has to lose its own gaps.
      if (guardianRows[0].parentElement === addGuardianHost()) {
        guardianRows[0].parentElement.classList.add("contour-person-group-host");
      }
    }
  ...
  /* =========================================================
     ADD GUARDIAN DETAILS — the Student flow's second person
     -----------------------------------------------------------
     A student signing up for themselves often has a parent who should be on
     the record: they are the ones who take the call, sign the forms and pay
     the invoice. Until now the only way to capture them was to fill the form
     in as the guardian, which a pre-fill link cannot offer at all — the
     "Are you a" question is hidden there, so the flow cannot be switched.

     So the guardian gets four dedicated boxes of his own — guardian_s_first_name,
     guardian_s_last_name, guardian_email, guardian_phone — rendered by HubSpot
     as dependents of web_form_contact_type = Student and therefore absent on
     the Guardian flow, where the visitor IS the guardian and the existing
     fields already hold them. Workflow A's identity resolution has read those
     four properties since it was written (the generic_student_dedicated_guardian
     mode), so nothing downstream changes: the student comes from the generic
     fields, the guardian from these, and Workflow B upserts and associates the
     guardian exactly as it does on the Guardian flow.

     THE FOUR WRAPPERS ARE NEVER MOVED. HubSpot renders them inside the contact
     type question's own .hs-dependent-field, and they are React's children of
     it: the embed removes them from that container the moment the flow
     switches, and a container of ours holding them makes its removeChild throw
     NotFoundError and unmount the whole form. Found the hard way on staging,
     22 Sep 2026 — it is the same rule the person group cards are built on.

     So the block is paint, not structure, exactly as those cards are: a prompt
     row and a "Guardian Details" band inserted as extra children of the same
     host — which HubSpot tolerates, since positionGuardianSegment() has been
     putting the visitor's own fieldsets in there on the Guardian flow all
     along — plus classes painted on the four wrappers where they stand, and
     inline display to open and shut them.

     Ordering falls out of the same move. HubSpot renders dependent fields
     ABOVE the visitor's own rows, so on the Student flow the visitor's two
     fieldsets step INSIDE the host, ahead of the guardian's boxes, instead of
     out of it — the mirror of what the Guardian flow already does with the
     student rows. See positionGuardianSegment().

     The fields are optional in HubSpot on purpose. A required field hidden
     this way blocks the submit with an error nobody can see (see the school
     placeholder), and pressing Remove empties all four so an abandoned
     half-answer cannot reach the CRM.
     ========================================================= */
  var ADD_GUARDIAN_CLASS = "contour-add-guardian";
  var ADD_GUARDIAN_ROW_CLASS = "contour-add-guardian__row";
  var ADD_GUARDIAN_ROW_SUFFIX_LEFT = ".contour-add-guardian__row--left";
  var ADD_GUARDIAN_ROW_SUFFIX_RIGHT = ".contour-add-guardian__row--right";
  var ADD_GUARDIAN_FIELD_NAMES = ["guardian_s_first_name", "guardian_s_last_name", "guardian_email", "guardian_phone"];
  // Same shape as PERSON_GROUPS entries, so setPersonFieldLabel() can shorten
  // these labels the same way: "First Name" on screen under a band that
  // already says Guardian, with the prefix kept in a visually-hidden span so
  // the error summary never shows two bare "First Name" chips.
  var ADD_GUARDIAN_FIELDS = [
    { selector: FIELD_SELECTORS.addGuardianFirstName, prefix: "Guardian", visible: "First Name" },
    { selector: FIELD_SELECTORS.addGuardianLastName, prefix: "Guardian", visible: "Last Name" },
    { selector: FIELD_SELECTORS.addGuardianEmail, prefix: "Guardian", visible: "Email" },
    { selector: FIELD_SELECTORS.addGuardianPhone, prefix: "Guardian", visible: "Phone Number" }
  ];
  var ADD_GUARDIAN_PROMPT_TITLE = "Add a parent or guardian";
  var ADD_GUARDIAN_PROMPT_NOTE = "We'll link them to your record so important updates reach them too.";
  var addGuardianOpen = false;
  /* What the record already has, as the pre-fill answered it.
       -1  no record was fetched — an ordinary visit, the block is the
           visitor's to open
        0  a record with no guardian on it: offer the prompt
        1  exactly one: show that guardian's details, filled in, and no way to
           take them off — an association already exists and this form cannot
           undo it. The fields stay editable, including the address; changing
           it adds a second guardian rather than renaming the first, which the
           note under the box says out loud
       2+  more than one: show nothing. There is no single row that represents
           them, and picking one to edit would quietly hide the others. */
  var addGuardianRecordCount = -1;
  // True while the fields hold a guardian the record supplied rather than one
  // the visitor typed. Only the visitor's own can be taken off again.
  var addGuardianPrefilled = false;
  var addGuardianPrefilledEmail = "";
  function addGuardianEnabled() {
    if (!featureEnabled("addGuardianOnStudentFlow") || isGuardianContactType()) return false;
    // Two or more guardians already: neither the prompt nor the box.
    return addGuardianRecordCount < 2;
  }
  // Called from the pre-fill once the record's associations are known.
  function applyAddGuardianRecord(guardian, count) {
    addGuardianRecordCount = typeof count === "number" ? count : (guardian ? 1 : 0);
    if (addGuardianRecordCount !== 1 || !guardian) return;
    addGuardianPrefilled = true;
    addGuardianPrefilledEmail = lowerTrim(guardian.email || guardian.email_2 || "");
    addGuardianOpen = true;
    setTextWhenPresent(FIELD_SELECTORS.addGuardianFirstName, guardian.firstname, 10);
    setTextWhenPresent(FIELD_SELECTORS.addGuardianLastName, guardian.lastname, 10);
    setTextWhenPresent(FIELD_SELECTORS.addGuardianEmail, guardian.email || guardian.email_2, 10);
    setPhoneValueWhenPresent(FIELD_SELECTORS.addGuardianPhone, guardian.phone, 10);
  }
  function lowerTrim(value) {
    return String(value || "").trim().toLowerCase();
  }
  // The address is left editable on purpose (Amrit, 22 Sep 2026): a student
  // whose parent has changed address should be able to say so. But changing it
  // does not rename the guardian on the record — upsertGuardian searches by
  // address, finds nothing, and creates a second one — so the swap is said out
  // loud rather than left to be discovered in the CRM.
  function refreshAddGuardianEmailNote() {
    var input = q(FIELD_SELECTORS.addGuardianEmail);
    if (!input) return;
    var wrap = fieldWrapper(input) || input.parentElement;
    if (!wrap) return;
    var existing = wrap.querySelector("." + ADD_GUARDIAN_CLASS + "__swap-note");
    var swapped = addGuardianPrefilled && addGuardianPrefilledEmail !== "" &&
      lowerTrim(input.value) !== "" && lowerTrim(input.value) !== addGuardianPrefilledEmail;
    if (!swapped) {
      if (existing && existing.parentNode) existing.parentNode.removeChild(existing);
      return;
    }
    ...
    note.className = "hs-field-desc " + ADD_GUARDIAN_CLASS + "__swap-note";
    note.setAttribute("role", "status");
    note.textContent = "A different address adds a second guardian rather than changing the one already on your record.";
    wrap.appendChild(note);
  }
  // All four or none: a partial set means HubSpot is mid-render, and painting
  // a box around two of them would leave the other two bare.
  function addGuardianWraps() {
    var wraps = [];
    for (var i = 0; i < ADD_GUARDIAN_FIELDS.length; i++) {
      var input = q(ADD_GUARDIAN_FIELDS[i].selector);
  ...
  // The .hs-dependent-field HubSpot renders the contact type question and its
  // dependents into. Everything this block injects goes in there beside them.
  function addGuardianHost() {
    var contact = q(FIELD_SELECTORS.contactType);
    var wrap = contact ? fieldWrapper(contact) : null;
    var host = wrap ? wrap.parentElement : null;
    return host && host.classList && host.classList.contains("hs-dependent-field") ? host : null;
  }
  function addGuardianChrome(role) {
    return formRoot ? formRoot.querySelector('[data-contour-add-guardian="' + role + '"]') : null;
  }
  function addGuardianHasValue() {
    for (var i = 0; i < ADD_GUARDIAN_FIELDS.length; i++) {
      var input = q(ADD_GUARDIAN_FIELDS[i].selector);
      if (!input) continue;
      var value = (input.value || "").trim();
      if (value === "") continue;
      // The phone widget seeds a dial code into an otherwise empty box, so
      // "+61" is the unanswered state rather than an answer.
      if (ADD_GUARDIAN_FIELDS[i].selector === FIELD_SELECTORS.addGuardianPhone && phoneNationalDigits(value) === "") continue;
      return true;
    }
    return false;
  }
  function clearAddGuardianValues() {
    ADD_GUARDIAN_FIELDS.forEach(function (config) {
      var input = q(config.selector);
      if (!input) return;
      var boxes = [input];
      var typable = typableInput(input);
      if (typable && typable !== input) boxes.push(typable);
      var group = input.closest ? input.closest(".contour-intl-phone, .hs-fieldtype-intl-phone") : null;
      var mirror = group ? group.querySelector('input[type="hidden"]') : null;
      ...
      // Whatever was showing about a value that no longer exists goes with it.
      clearContourError(input, "contour-add-guardian-firstname-error");
      clearContourError(input, "contour-add-guardian-lastname-error");
      clearContourError(input, "contour-add-guardian-email-error");
      input.classList.remove("invalid", "error");
      if (typable && typable !== input) typable.classList.remove("invalid", "error");
    });
    updateInjectedPhoneError(INJECTED_PHONE_FIELDS[1]);
    refreshAllContactFormatErrors();
  }
  function openAddGuardian(focusFirst) {
    addGuardianOpen = true;
    renderAddGuardianState();
    if (focusFirst === false) return;
    var first = q(FIELD_SELECTORS.addGuardianFirstName);
    if (!first) return;
    try {
      first.focus();
    } catch (err) { }
  ...
  function closeAddGuardian() {
    if (addGuardianPrefilled) return;
    addGuardianOpen = false;
    clearAddGuardianValues();
    renderAddGuardianState();
    var prompt = addGuardianChrome("prompt");
    var cta = prompt ? prompt.querySelector("." + ADD_GUARDIAN_CLASS + "__cta") : null;
    if (!cta) return;
    try {
      cta.focus();
    } catch (err) { }
  }
  // Re-asserted on every pass, not written once: HubSpot replaces these
  // wrappers whenever its own validation fires, and a fresh wrapper arrives
  // without the inline display that was keeping it shut.
  /* The email recognition panel — "we have this as a guardian's address", "is
     this you?" — renders in the same place, in the same grey, at the same
     width. Two of them stacked read as one control that has been drawn twice,
     and the visitor has to work out which question is theirs to answer.

     The panel wins: it is a question about the address they have just typed,
     and it has to be settled before adding anyone else is a sensible offer.
     The prompt comes back the moment it goes (Amrit, 23 Sep 2026). */
  function studentRecognitionPanelShowing() {
    if (!formRoot) return false;
    var panel = formRoot.querySelector("." + STUDENT_RECOGNITION_PANEL_CLASS);
    // An empty panel is one this file has built but not filled — it holds no
    // question, so it is not competing with anything.
    return !!panel && panel.childNodes.length > 0 && isElementVisible(panel);
  }
  function renderAddGuardianState() {
    var prompt = addGuardianChrome("prompt");
    var band = addGuardianChrome("band");
    var wraps = addGuardianWraps();
    if (!prompt || !band || wraps.length === 0) return;
    prompt.style.display = addGuardianOpen || studentRecognitionPanelShowing() ? "none" : "";
    band.style.display = addGuardianOpen ? "" : "none";
    // A guardian the record supplied is already associated, and nothing this
    // form does can undo that — so there is no cross to offer. One the visitor
    // added in this session is theirs to take off again.
    var close = band.querySelector("." + ADD_GUARDIAN_CLASS + "__close");
    if (close) close.style.display = addGuardianPrefilled ? "none" : "";
    if (addGuardianOpen) syncAddGuardianBandRule(band);
    refreshAddGuardianEmailNote();
    wraps.forEach(function (wrap) {
      // display, not just an attribute: isElementVisible() walks the inline
      // display flags, and that walk is what keeps a shut field out of the
      // submit gate, the deliverability lookup and the error summary.
      if (addGuardianOpen) wrap.style.removeProperty("display");
      else wrap.style.display = "none";
    });
  }
  function addGuardianIcon() {
    var svg = document.createElementNS("http://www.w3.org/2000/svg", "svg");
    svg.setAttribute("viewBox", "0 0 24 24");
    svg.setAttribute("aria-hidden", "true");
    svg.setAttribute("focusable", "false");
    var path = document.createElementNS("http://www.w3.org/2000/svg", "path");
    // A head and shoulders with a plus beside it, drawn as strokes so it
    // inherits the navy the rest of the row is set in.
    path.setAttribute("d", "M9.5 11.5a3.5 3.5 0 1 0 0-7 3.5 3.5 0 0 0 0 7Zm0 1.75c-3.2 0-5.75 1.7-5.75 3.8V19h11.5v-1.95c0-2.1-2.55-3.8-5.75-3.8ZM18 8.5v5M15.5 11h5");
  ...
  function buildAddGuardianPrompt() {
    var prompt = document.createElement("div");
    prompt.setAttribute("data-contour-add-guardian", "prompt");
    prompt.className = ADD_GUARDIAN_CLASS + "__prompt";
    var icon = document.createElement("span");
    icon.className = ADD_GUARDIAN_CLASS + "__icon";
    icon.appendChild(addGuardianPlusIcon());
    var copy = document.createElement("div");
    copy.className = ADD_GUARDIAN_CLASS + "__copy";
    var title = document.createElement("span");
    title.className = ADD_GUARDIAN_CLASS + "__title";
    title.textContent = ADD_GUARDIAN_PROMPT_TITLE;
    var note = document.createElement("span");
    note.className = ADD_GUARDIAN_CLASS + "__note";
    note.textContent = ADD_GUARDIAN_PROMPT_NOTE;
    copy.appendChild(title);
    copy.appendChild(note);
    // The button sits behind a hairline the same 24px off as the recognition
    // panel's "That's me", so the two rows read as one pattern.
    var actions = document.createElement("div");
    actions.className = ADD_GUARDIAN_CLASS + "__actions";
    var cta = document.createElement("button");
    cta.type = "button";
    cta.className = ADD_GUARDIAN_CLASS + "__cta";
    // The person-with-a-plus goes on the button and the bare plus on the
    // badge, not the other way round: the button is the action, so it wears
    // the thing being added (Amrit, 23 Sep 2026).
    cta.appendChild(addGuardianIcon());
    cta.appendChild(document.createElement("span")).textContent = "Add Guardian/Parent";
    cta.addEventListener("click", function () {
      openAddGuardian(true);
    });
  ...
  // Built out of the person segment band's own classes rather than a lookalike:
  // the pill, the tapering rule and the 30px band come from
  // injectPersonGroupStyles and the section box rules, so "Guardian Details"
  // lands on the same grid line as "Student Details" above it and moves with it
  // if either is ever restyled (Amrit, 22 Sep 2026).
  function buildAddGuardianBand() {
    var band = document.createElement("div");
    band.setAttribute("data-contour-add-guardian", "band");
    band.className = "contour-person-tabs contour-person-tabs--static contour-person-tabs--mid " + ADD_GUARDIAN_CLASS + "__band";
    var heading = document.createElement("span");
    heading.className = "contour-person-tabs__heading";
    var label = document.createElement("span");
    label.className = "contour-person-tabs__label";
    label.textContent = headerLabel("Guardian Details");
    heading.appendChild(label);
    var close = document.createElement("button");
    close.type = "button";
    close.className = ADD_GUARDIAN_CLASS + "__close";
    // The cross alone was a 22px target and too quiet to read as a control.
    // It stays wordless — the ring around it is what marks it as a button, and
    // a label beside the pill competed with it (Amrit, 23 Sep 2026). The name
    // is carried for screen readers only.
    close.setAttribute("aria-label", "Remove guardian details");
    close.setAttribute("title", "Remove guardian details");
    close.appendChild(addGuardianCrossIcon());
    close.addEventListener("click", closeAddGuardian);
    band.appendChild(heading);
    band.appendChild(close);
    return band;
  }
  function addGuardianPlusIcon() {
    var svg = document.createElementNS("http://www.w3.org/2000/svg", "svg");
    ...
    path.setAttribute("fill", "none");
    path.setAttribute("stroke", "currentColor");
    // Heavier than the drawn glyphs elsewhere: knocked out white on the blue
    // disc a 2-unit stroke reads thin at 32px.
    path.setAttribute("stroke-width", "2.6");
    path.setAttribute("stroke-linecap", "round");
    svg.appendChild(path);
    return svg;
  }
  function addGuardianCrossIcon() {
    var svg = document.createElementNS("http://www.w3.org/2000/svg", "svg");
    svg.setAttribute("viewBox", "0 0 16 16");
    svg.setAttribute("aria-hidden", "true");
    svg.setAttribute("focusable", "false");
  ...
  // Idempotent, and it only ever touches the tree when the order is already
  // wrong: this runs from a childList MutationObserver, so a write on every
  // pass would retrigger the observer forever.
  function placeAddGuardianChrome(node, host, anchor) {
    if (node.parentElement !== host || node.nextSibling !== anchor) host.insertBefore(node, anchor);
  }
  // The rule starts past the pill, and the pill's width is copy. Measured the
  // same way ensurePersonStaticHeader measures its own, and skipped while the
  // band is laid out at zero width — a later pass, once it is on screen, sets
  // the real number.
  function syncAddGuardianBandRule(band) {
    band.classList.toggle("contour-person-tabs--label-left", !featureEnabled("personLabelRight"));
    var heading = band.querySelector(".contour-person-tabs__heading");
    var width = heading ? heading.offsetWidth : 0;
    if (width > 0) band.style.setProperty("--contour-seg-rule-start", (width + 10) + "px");
    else band.style.removeProperty("--contour-seg-rule-start");
    // And stop short of the close control rather than running under it. Its
    // width is copy too, so it is measured the same way the pill is.
    var close = band.querySelector("." + ADD_GUARDIAN_CLASS + "__close");
    var closeWidth = close && close.style.display !== "none" ? close.offsetWidth : 0;
    if (closeWidth > 0) band.style.setProperty("--contour-add-guardian-rule-end", (closeWidth + 14) + "px");
    else band.style.removeProperty("--contour-add-guardian-rule-end");
  }
  function teardownAddGuardian() {
    ["prompt", "band"].forEach(function (role) {
      var node = addGuardianChrome(role);
      if (node && node.parentNode) node.parentNode.removeChild(node);
    });
    // Whatever is left of the paint comes off the wrappers, for the case where
    // they are still standing — the flag turned off rather than the flow
    // switched, where HubSpot has taken them away itself.
    qAll("." + ADD_GUARDIAN_ROW_CLASS).forEach(function (wrap) {
      wrap.classList.remove(ADD_GUARDIAN_ROW_CLASS, ADD_GUARDIAN_ROW_CLASS + "--left", ADD_GUARDIAN_ROW_CLASS + "--right", ADD_GUARDIAN_ROW_CLASS + "--last");
      wrap.style.removeProperty("display");
    });
    addGuardianOpen = false;
  }
  function updateAddGuardian() {
    if (!formRoot) return;
    if (!addGuardianEnabled()) {
      teardownAddGuardian();
      return;
    }
    var host = addGuardianHost();
    var wraps = addGuardianWraps();
    // Mid-render, or a form that has not been published with the four fields
    // yet: leave the page exactly as it is. The contact-field MutationObserver
    // runs this again the moment they arrive.
    if (!host || wraps.length === 0 || wraps[0].parentElement !== host) {
      teardownAddGuardian();
      return;
    }
    injectAddGuardianStyles();
    var prompt = addGuardianChrome("prompt") || buildAddGuardianPrompt();
    var band = addGuardianChrome("band") || buildAddGuardianBand();
    // Band first, then the prompt in front of it: anchoring both on the same
    // node makes each insert move the other's next sibling, and this runs from
    // a childList observer that would then chase its own tail forever.
    placeAddGuardianChrome(band, host, wraps[0]);
    placeAddGuardianChrome(prompt, host, band);
    syncAddGuardianBandRule(band);
    wraps.forEach(function (wrap, index) {
      if (!wrap.classList.contains(ADD_GUARDIAN_ROW_CLASS)) wrap.classList.add(ADD_GUARDIAN_ROW_CLASS);
      // Which side wall a row carries is its position among the FOUR, and
      // nothing in CSS can count that: :nth-of-type would count the contact
      // type question, the prompt and the band as well, since all three are
      // divs in the same host, and every row would take the wrong wall.
      wrap.classList.toggle(ADD_GUARDIAN_ROW_CLASS + "--left", index % 2 === 0);
      wrap.classList.toggle(ADD_GUARDIAN_ROW_CLASS + "--right", index % 2 === 1);
      wrap.classList.toggle(ADD_GUARDIAN_ROW_CLASS + "--last", index === wraps.length - 1);
    });
    // A value that arrived without the prompt being pressed — a restored
    // draft, a browser autofill — opens the block rather than hiding an
    // answer the visitor can no longer see or correct.
    if (!addGuardianOpen && addGuardianHasValue()) addGuardianOpen = true;
    ADD_GUARDIAN_FIELDS.forEach(function (config) {
      setPersonFieldLabel(config, true);
    });
    bindAddGuardianEmailWatch();
    renderAddGuardianState();
  }
  var ADD_GUARDIAN_EMAIL_BOUND_ATTR = "data-contour-add-guardian-email";
  function bindAddGuardianEmailWatch() {
    var input = q(FIELD_SELECTORS.addGuardianEmail);
    if (!input || input.getAttribute(ADD_GUARDIAN_EMAIL_BOUND_ATTR) === "1") return;
    input.setAttribute(ADD_GUARDIAN_EMAIL_BOUND_ATTR, "1");
    input.addEventListener("input", refreshAddGuardianEmailNote);
    input.addEventListener("blur", refreshAddGuardianEmailNote);
  }
  function injectAddGuardianStyles() {
    if (document.getElementById("contour-add-guardian-styles")) return;
    var navy = "#0C3166";
    var line = "rgba(12, 49, 102, 0.12)";
    var muted = "rgba(12, 49, 102, 0.62)";
    var wash = "rgba(12, 49, 102, 0.045)";
    var danger = "#C8102E";
    var block = ".hs-form ." + ADD_GUARDIAN_CLASS;
    var row = ".hs-form ." + ADD_GUARDIAN_ROW_CLASS;
    var style = document.createElement("style");
    style.id = "contour-add-guardian-styles";
    style.textContent = "" +
      /* --- the resting prompt ---------------------------------------------
         A quiet grey panel rather than another white card: the contact rows
         around it are already white on the card, and a third white box read as
         a field group the visitor had missed rather than an offer (Amrit,
         22 Sep 2026). */
      block + "__prompt { display: flex; align-items: center; gap: 14px; box-sizing: border-box; width: 100%; flex: 0 0 100%; grid-column: 1 / -1; margin: 18px 0 30px; padding: 16px 20px; background: " + wash + "; border: 1px solid " + line + "; border-radius: 16px; transition: background-color .15s ease, border-color .15s ease; }" +
      block + "__prompt:hover { background: rgba(12, 49, 102, 0.06); border-color: rgba(12, 49, 102, 0.20); }" +
      /* The badge is a rounded rectangle wider than it is tall, ringed the way
         the segment pills are, so it reads as a piece of the same set rather
         than a stray avatar. */
      // The recognition panel's disc, to the pixel: 32px, a 2px navy ring and
      // the same #007AFF fill, white glyph. It is a badge, not a control, so
      // nothing happens to it on hover (Amrit, 23 Sep 2026).
      block + "__icon { display: inline-flex; align-items: center; justify-content: center; flex: 0 0 auto; width: 32px; height: 32px; box-sizing: border-box; border: 2px solid " + navy + "; border-radius: 50%; background: #007AFF; color: #FFFFFF; }" +
      block + "__icon svg { width: 17px; height: 17px; display: block; }" +
      block + "__copy { display: flex; flex-direction: column; gap: 2px; min-width: 0; margin-right: auto; }" +
      block + "__title { font-size: 15px; font-weight: 600; line-height: 1.3; color: " + navy + "; }" +
      block + "__note { font-size: 13px; line-height: 1.4; color: " + muted + "; }" +
      // Filled navy, white label, a plus ahead of the word — the page's own
      // primary button, so the offer reads as the action it is.
      // The hairline the recognition panel sets its own button behind.
      block + "__actions { display: flex; align-items: center; flex: 0 0 auto; align-self: stretch; padding-left: 24px; border-left: 1px solid rgba(12, 49, 102, 0.12); }" +
      // Lifted straight off __send--switch, so "Add Guardian/Parent" and
      // "That's me" lighten and glow by the same amounts.
      block + "__cta { display: inline-flex; align-items: center; gap: 8px; white-space: nowrap; flex: 0 0 auto; appearance: none; -webkit-appearance: none; font: inherit; font-size: 14px; font-weight: 700; line-height: 1.3; padding: 10px 20px; border-radius: 999px; border: 0; background: " + navy + "; color: #FFFFFF; cursor: pointer; transition: background-color .15s ease, transform .15s ease, box-shadow .15s ease; }" +
      block + "__cta:hover { background: #164C93; transform: translateY(-1px); box-shadow: 0 4px 12px rgba(12, 49, 102, 0.22); }" +
      block + "__cta:active { transform: translateY(0); box-shadow: 0 1px 4px rgba(12, 49, 102, 0.20); }" +
      block + "__cta:focus-visible { outline: none; box-shadow: 0 0 0 3px rgba(12, 49, 102, 0.30); }" +
      block + "__cta svg { width: 17px; height: 17px; display: block; }" +
      /* --- the opened section ----------------------------------------------
         No box of its own. The band is the person segment band — pill plus
         tapering rule — and the four rows are laid out exactly as the Student
         Details rows above them, so the two segments read as one list of
         people rather than a card inside a card. */
      block + "__band { justify-content: flex-start; }" +
      // The rule stops short of the cross instead of running under it.
      ".hs-form .contour-section-box ." + ADD_GUARDIAN_CLASS + "__band::before { right: var(--contour-add-guardian-rule-end, 30px); }" +
      // An outline pill in the red the cross is drawn in — the same shape the
      // segment labels wear, so it belongs on the band it sits in.
      // A round button with a ring, no word: the ring is what says "control",
      // and it sits on the band as its own shape rather than as a second label
      // beside the pill. 28px is the target; the cross inside stays 13.
      block + "__close { display: inline-flex; align-items: center; justify-content: center; margin-left: auto; flex: 0 0 auto; width: 28px; height: 28px; padding: 0; box-sizing: border-box; appearance: none; -webkit-appearance: none; border: 1px solid rgba(200, 16, 46, 0.45); border-radius: 50%; background: transparent; color: " + danger + "; cursor: pointer; transition: background-color .15s ease, border-color .15s ease; }" +
      block + "__close:hover { background: rgba(200, 16, 46, 0.08); border-color: " + danger + "; }" +
      block + "__close:focus-visible { outline: none; box-shadow: 0 0 0 3px rgba(200, 16, 46, 0.25); }" +
      block + "__close svg { width: 13px; height: 13px; display: block; }" +
      // Measured off the live Student Details rows so the columns line up.
      row + " { flex: 0 0 50% !important; box-sizing: border-box; margin: 0 !important; padding: 12px 12px 18px 0; }" +
      row + "--right { padding: 12px 0 18px 12px; }" +
      // HubSpot's own 8px gutter on .input is zeroed for the two-column
      // fieldsets by the page stylesheet, but these rows are not in one — left
      // alone their boxes came up 8px short of the Student Details boxes above.
      row + " > .input { margin-right: 0 !important; }" +
      row + "--last { margin-bottom: 8px !important; }" +
      block + "__swap-note { margin-top: 6px; font-size: 12.5px; line-height: 1.4; color: " + muted + "; }" +
      /* On a phone the rows stack full width and the prompt becomes badge +
         copy on one line with the button under them. Left as a wrapping flex
         row the badge sat alone on a line of its own above the title.

         The copy runs to three lines at 375px, and centring the badge against
         that block left it floating beside the middle line, detached from the
         heading it belongs to — so it aligns to the top instead. The gaps and
         the panel's own padding open up with it: at 12px the button sat hard
         against the copy above it and close to the panel edge (Amrit,
         23 Sep 2026). */
      "@media screen and (max-width: 767px) {" +
      row + ", " + row + "--right { flex: 0 0 100% !important; padding: 10px 0 14px; }" +
      block + "__prompt { display: grid; grid-template-columns: auto 1fr; column-gap: 14px; row-gap: 18px; align-items: start; padding: 18px; }" +
      block + "__actions { grid-column: 1 / -1; padding-left: 0; border-left: 0; }" +
      block + "__cta { width: 100%; justify-content: center; padding: 12px 20px; }" +
      block + "__band { padding-right: 0; }" +
      "}";
    document.head.appendChild(style);
  }
  // The prompt's badge sits on the card, which is white — named here so the
  // one token is not spelled out twice.
  function cardBgToken() {
    return "#FFFFFF";
  }
  ...
  function injectPersonGroupStyles() {
    ...
    style.textContent = "" +
      ...
      // HubSpot's own sheet gutters the two columns with
      // `fieldset.form-columns-2 .input { margin-right: 8px }`. On a plain
      // text field that 8px is invisible: the input is content-box, so its
      // own 2px border and 2px padding on each side put the border box back
      // out to the field's content edge. The intl-phone widget has both of
      // those zeroed, so it kept the 8px and stopped short — the right-hand
      // column's fields ended 8px inside every other field's right edge,
      // while the left-hand column stayed flush. Invisible until the
      // recognition panel ran the full width of the card and gave the eye a
      // straight edge to compare against (Amrit, 18 Sep 2026).
      //
      // The gutter is already carried by the fields' own 12px padding, so
      // the margin goes; border-box then holds every input to its field's
      // content edge without leaning on the border-and-padding coincidence.
      ".hs-form fieldset.contour-person-card__row--guardian > .hs-form-field > .input { margin-right: 0 !important; }" +
      ".hs-form fieldset.contour-person-card__row--guardian .hs-input:not([type=\"checkbox\"]):not([type=\"radio\"]):not([type=\"file\"]) { box-sizing: border-box; }" +
  ...
  function evaluateProgramInterestOptions() {
    ...
    if (featureEnabled("autoSelectAllPrograms") && !programCardsTouched && eligibleOptions.length > 0) {
      // Programmatic ticks, so the change handler below does not read them
      // as the visitor's own and lock the defaults in. Each click rebuilds
      // the subject list and the nudge row through the ordinary handlers.
      eligibleOptions.forEach(function (opt) {
        if (!opt.checked) setCheckboxChecked(opt, true);
      });
      anyChecked = true;
    }
  ...
  // True once the visitor ticks or unticks a program card themselves (a
  // trusted change outside a programmatic edit, or the nudge row's button).
  // From then on the eligible set may change but nothing is ticked for them.
  var programCardsTouched = false;
  ...
  /* =========================================================
     PROGRAM NUDGE
     -----------------------------------------------------------
     One row per program that is eligible for the visitor's location, year
     level and intake year but not ticked, shown only once at least one
     program IS ticked. Zero ticked belongs to the required-field error;
     every eligible program ticked leaves nothing to suggest. Each row is a
     sentence in the brand's voice and a "Show subjects" button that ticks
     the card through its own click, so the change handlers, the section
     flow and the draft cache all see an ordinary user pick.

     Sits directly under the card list, above the coverage error, inside
     the program field's wrap. Pre-fill links land here too: a record that
     already holds one program gets the same suggestion for the others.
     ========================================================= */
  var PROGRAM_NUDGE_COPY = {
    Education: "We also offer high school tutoring for your location.",
    MedPrep: "We also offer med school entry prep (UCAT & interview) for your location.",
    TestPrep: "We also offer selective & scholarship school entry prep for your location."
  };
  // Button label names the brand the way its wordmark does (Amrit, 21 Sep 2026).
  var PROGRAM_NUDGE_CTA = {
    Education: "View Education subjects",
    MedPrep: "View MedPrep subjects",
    TestPrep: "View TestPrep subjects"
  };
  // Row prefix: open book, stroke in currentColor, sized for the 30px badge
  // so it weighs the same as the button at the row's other end.
  var PROGRAM_NUDGE_ROW_ICON = '<svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" focusable="false"><path d="M12 6.5C10.5 5.3 8.6 4.8 6.5 4.8H3v13.7h3.5c2.1 0 4 .5 5.5 1.7 1.5-1.2 3.4-1.7 5.5-1.7H21V4.8h-3.5c-2.1 0-4 .5-5.5 1.7z"/><path d="M12 6.5v13.7"/></svg>';
  function injectProgramNudgeStyles() {
    if (document.getElementById("contour-program-nudge-styles")) return;
    var style = document.createElement("style");
    style.id = "contour-program-nudge-styles";
    style.textContent = "" +
      // Same voice as the school "not found" hint: soft blue field with a
      // blue outline, navy text, 14px semibold (a step up from the hint's
      // 13px so it holds its own beside the 30px badge and the button). A grey field was tried and
      // put back — it read as disabled next to the cards (Amrit, 21 Sep
      // 2026). 12px above matches the coverage error's clearance from the
      // card row.
      ".hs-form .contour-program-nudge { display: flex; flex-direction: column; gap: 8px; margin: 12px 0 0; padding: 10px 12px; border: 1px solid #cfe0ff; border-radius: 10px; background: #F2F7FF; color: #0C3166; }" +
      ".hs-form .contour-program-nudge__row { display: flex; align-items: center; gap: 10px; margin: 0; font-size: 14px; line-height: 1.45; font-weight: 600; }" +
      // Badge in the selected card's blue with a navy ring, holding a white
      // book glyph — "there are subjects here", not "information".
      ".hs-form .contour-program-nudge__icon { flex: 0 0 auto; display: flex; align-items: center; justify-content: center; width: 30px; height: 30px; border-radius: 50%; background: #005FCC; color: #FFFFFF; box-shadow: inset 0 0 0 1.5px #0C3166; }" +
      ".hs-form .contour-program-nudge__icon svg { display: block; }" +
      // Basis 0, not auto: when the row wraps on a phone the sentence must
      // shrink to fit beside the disc instead of dropping under it.
      ".hs-form .contour-program-nudge__text { flex: 1 1 0%; min-width: 0; }" +
      // Solid navy pill with a white label — the email recognition panel's
      // navy "switch" button, so the two navy CTAs on the form move the same
      // way: hover lightens it a step, lifts it 1px and puts a soft glow
      // under it; press settles it; keyboard focus gets the panel's ring.
      // A blue fill with a navy outline that flooded in on hover was tried
      // and put back — it read as a twin of the badge (Amrit, 21 Sep 2026).
      ".hs-form .contour-program-nudge__button { flex: 0 0 auto; display: inline-flex; align-items: center; appearance: none; -webkit-appearance: none; margin: 0; border: 0; border-radius: 999px; background: #0C3166; color: #FFFFFF; padding: 8px 16px; font: inherit; font-size: 13px; font-weight: 600; line-height: 1.3; white-space: nowrap; cursor: pointer; transition: background-color .15s ease, transform .15s ease, box-shadow .15s ease; }" +
      ".hs-form .contour-program-nudge__button:hover { background: #164C93; transform: translateY(-1px); box-shadow: 0 4px 12px rgba(12,49,102,0.22); }" +
      ".hs-form .contour-program-nudge__button:active { transform: translateY(0); box-shadow: 0 1px 4px rgba(12,49,102,0.20); }" +
      ".hs-form .contour-program-nudge__button:focus-visible { outline: none; box-shadow: 0 0 0 3px rgba(12,49,102,0.30); }" +
      // Narrow phones: the sentence keeps the icon beside it, the button
      // drops to its own line under the text.
      "@media (max-width: 480px) { .hs-form .contour-program-nudge__row { flex-wrap: wrap; } .hs-form .contour-program-nudge__button { flex-basis: 100%; justify-content: center; } }";
    document.head.appendChild(style);
  }
  function ensureProgramNudge() {
    var existing = formRoot.querySelector("#contour-program-nudge");
    if (existing) return existing;
    var first = q(FIELD_SELECTORS.programInterest);
    var fieldWrap = first ? fieldWrapper(first) : null;
    if (!fieldWrap) return null;
    injectProgramNudgeStyles();
    var box = document.createElement("div");
    box.id = "contour-program-nudge";
    box.className = "contour-program-nudge";
    box.style.display = "none";
    box.setAttribute("aria-label", "Other programs available to you");
    // Under the card list itself, not appended to the wrap: the coverage
    // error is appended there at init and must stay below this.
    var cardList = first.closest("ul") || first.closest(".contour-program-card-list");
    if (cardList && cardList.parentNode) cardList.parentNode.insertBefore(box, cardList.nextSibling);
    else fieldWrap.appendChild(box);
    return box;
  }
  function renderProgramNudge() {
    if (!featureEnabled("programNudge")) return;
    var box = ensureProgramNudge();
    if (!box) return;
    var options = qAll(FIELD_SELECTORS.programInterest);
    var anyChecked = options.some(function (opt) {
      return opt.checked;
    });
    var missing = anyChecked ? options.filter(function (opt) {
      return !opt.checked && !opt.disabled && PROGRAM_NUDGE_COPY[opt.value];
    }) : [];
    ...
    missing.forEach(function (opt) {
      ...
      row.className = "contour-program-nudge__row";
      row.setAttribute("data-program", opt.value);
      var icon = document.createElement("span");
      icon.className = "contour-program-nudge__icon";
      icon.setAttribute("aria-hidden", "true");
      icon.innerHTML = PROGRAM_NUDGE_ROW_ICON;
      row.appendChild(icon);
      var text = document.createElement("span");
      text.className = "contour-program-nudge__text";
      text.textContent = PROGRAM_NUDGE_COPY[opt.value];
      row.appendChild(text);
      var button = document.createElement("button");
      button.type = "button";
      button.className = "contour-program-nudge__button";
      button.textContent = PROGRAM_NUDGE_CTA[opt.value] || "View subjects";
      button.setAttribute("aria-label", "Show " + (PROGRAM_DISPLAY_NAMES[opt.value] || opt.value) + " subjects");
      button.addEventListener("click", function () {
        if (opt.checked || opt.disabled) return;
        // The visitor's own choice, so not wrapped as a programmatic edit —
        // the section flow and the draft cache should treat it as a tick on
        // the card. click() rather than a checked= write so the change
        // handlers that rebuild the subject list run.
        clickWithoutMovingPage(opt);
      });
  ...
  function getUrlParam(name, anyCase) {
    var match = new RegExp("[?&]" + name + "=([^&#]*)", anyCase ? "i" : "").exec(window.location.search);
    return match ? decodeURIComponent(match[1].replace(/\+/g, " ")) : "";
  }
  // A referral partner's link carries their code:
  // /free-trial?REFERRALCODE=CONTOUR123. The field it lands in is hidden on
  // the form, so the link is its only source — nobody can type one in without
  // one. Read case-insensitively because the parameter is hand-written into
  // campaign links (Amrit, 20 Sep 2026).
  var REFERRAL_CODE_PARAM = "REFERRALCODE";
  var REFERRAL_CODE_LINKED_CLASS = "contour-referral-code--from-link";
  var REFERRAL_CODE_BADGE_CLASS = "contour-referral-code__badge";
  function referralCodeFromUrl() {
    return getUrlParam(REFERRAL_CODE_PARAM, true).trim();
  }
  // Silently filling a hidden field leaves the visitor with no way to tell
  // whether their partner's link worked, so a link that carries a code brings
  // the row out of hiding with the code in it. Answering a question they were
  // never asked needs a word about where the answer came from — hence the
  // badge on the value (Amrit, 20 Sep 2026).
  function injectReferralCodeStyles() {
    if (document.getElementById("contour-referral-code-styles")) return;
    var style = document.createElement("style");
    style.id = "contour-referral-code-styles";
    var box = ".hs-form ." + REFERRAL_CODE_LINKED_CLASS + " .input";
    var badge = ".hs-form ." + REFERRAL_CODE_BADGE_CLASS;
    style.textContent = "" +
      // The pill rides inside the field box rather than up on the label row,
      // where it read as a stray button floating off the corner of the form.
      // Sitting on the value it is talking about, it is a state badge on that
      // one answer and can be taken in without moving the eye (Amrit, 20 Sep
      // 2026).
      box + " { position: relative; }" +
      // #005FCC, the blue a picked campus card wears, rather than the navy:
      // navy is what the form's own furniture is made of — section bands,
      // labels, the selected radio — so a navy badge read as more furniture.
      // The medium blue is the colour this form uses for "you chose this"
      // (Amrit, 20 Sep 2026).
      badge + " { position: absolute; top: 50%; right: 14px; transform: translateY(-50%); pointer-events: none; padding: 4px 12px; border-radius: 999px; background: #005FCC; color: #FFFFFF; font: inherit; font-size: 12px; font-weight: 700; line-height: 1.5; letter-spacing: 0.01em; white-space: nowrap; }" +
      // The badge overlays the box, so the value needs a lane of its own to
      // end in — a long code would otherwise run under the blue.
      box + " .hs-input { padding-right: 104px !important; }";
    document.head.appendChild(style);
  }
  // Idempotent by design: the init retry below calls it against a form that
  // HubSpot is still hydrating, and refreshDerivedFieldState calls it again
  // after every re-render that could have taken the row back out.
  function syncReferralCodeField() {
    var code = referralCodeFromUrl();
    if (!code) return;
    var field = q(FIELD_SELECTORS.referralCodeEntered);
    if (!field) return;
    // A property marked hidden in HubSpot comes down as <input type="hidden">,
    // which has no box to show a code in however visible its row is. The type
    // flips before the value goes in, since a type change can drop it.
    if (field.type === "hidden") field.type = "text";
    if (field.value !== code) setSelectOrTextValue(FIELD_SELECTORS.referralCodeEntered, code);
    injectReferralCodeStyles();
    var wrap = fieldWrapper(field);
    if (wrap) wrap.classList.add(REFERRAL_CODE_LINKED_CLASS);
    // Re-created rather than moved if a re-render took the box with it —
    // this runs on every refresh pass.
    var box = wrap ? wrap.querySelector(".input") : null;
    if (box && !box.querySelector("." + REFERRAL_CODE_BADGE_CLASS)) {
      var badge = document.createElement("span");
      badge.className = REFERRAL_CODE_BADGE_CLASS;
      badge.textContent = "Applied";
      box.appendChild(badge);
    }
    // Brings the row's own fieldset back with it — the field is the only one
    // in that row, so HubSpot's hidden flag had collapsed the whole thing.
    showFieldWrapper(field);
  }
  function applyReferralCodeFromUrl(tries) {
    if (tries === undefined) tries = 8;
    if (!referralCodeFromUrl()) return;
    // Same hydration race as defaultContactTypeToStudent: HubSpot re-renders
    // just after onFormReady and drops a synchronous write, so the value goes
    // in on a delay and is put back on every pass that finds it gone.
    setTimeout(function () {
      syncReferralCodeField();
      if (tries > 0) applyReferralCodeFromUrl(tries - 1);
    }, 250);
  }
  ...
  function initPrefetchFromUrl() {
    ...
    urlPrefetchPromise.then(function (data) {
      ...
      if (prefilled) {
        ...
        // visitor themselves, and applyPrefill has just put them in their own
        // fields.
        if (data.contact.contact_type !== "Guardian" && data.contact.contact_type !== "Parent") {
          applyAddGuardianRecord(data.guardian, data.guardianCount);
        }
  ...
  function draftPrefillMeta() {
    ...
    return {
      ...
      // How many guardians the record already had, and whether the ones in the
      // boxes came from it. Without these a refresh would offer a cross over a
      // guardian who is already associated — and this form cannot undo an
      // association, so the cross would be a lie.
      guardians: addGuardianRecordCount,
      guardianFromRecord: addGuardianPrefilled,
      guardianEmail: addGuardianPrefilledEmail
    };
  }
  ...
  function writeDraft(values) {
    ...
    // the ticks: a draft holding two of three programs could be defaults
    // (third ineligible) or a deliberate untick. Without this a refresh
    // re-ticked what they had just removed (Amrit, 21 Sep 2026).
    if (programCardsTouched) payload.programsTouched = true;
    try {
  ...
  function collectDraftValues() {
    ...
    draftFieldNames().forEach(function (name) {
      ...
      if (value === "") return;
      if (name === "phone" || name === "student_phone_number" || name === "guardian_phone") {
        var parts = splitE164(value);
  ...
  function applyDraftValue(name, value) {
    ...
    // The guardian block is shut by default, so a restored answer would go
    // into a box nobody can see — open it first, then fill.
    if (ADD_GUARDIAN_FIELD_NAMES.indexOf(name) !== -1) openAddGuardian(false);
    if (name === "guardian_phone") {
      setPhoneValueWhenPresent(selector, value, 10);
      return;
    }
  ...
  function initDraftRestore() {
    ...
    if (entry.prefill) {
      ...
      if (typeof entry.prefill.guardians === "number") addGuardianRecordCount = entry.prefill.guardians;
      if (entry.prefill.guardianFromRecord) {
        addGuardianPrefilled = true;
        addGuardianPrefilledEmail = String(entry.prefill.guardianEmail || "").trim().toLowerCase();
      }
    ...
    // collapses the cards on its own before they can be applied. Same for
    // the program cards: the matrix values coming back run the default-tick
    // pass, which must already know the visitor had taken the cards over.
    restoreSectionBoxState(entry.boxes);
    if (entry.programsTouched) programCardsTouched = true;
    restoreDraft(values);
    enforcePrefilledFieldLock();
  ...
  function attachListeners() {
    ...
    qAll(FIELD_SELECTORS.programInterest).forEach(function (el) {
      el.addEventListener("change", function () {
        if (!isProgrammaticEdit()) programCardsTouched = true;
        evaluateInterestedSubjectsOptions();
        evaluateCampusOptions();
        renderProgramNudge();
        renderWelcomeConsultation();
  ...
  var SECTION_DEFS = [{
    ...
    fields: ["web_form_contact_type", "student_first_name", "student_last_name", "student_email", "student_phone_number", "firstname", "lastname", "email_2", "phone", "guardian_s_first_name", "guardian_s_last_name", "guardian_email", "guardian_phone"]
  }, {
    id: "study",
    ...
    id: "finish",
    title: "Final Details",
    fields: ["referral", "referral_code_entered", "how_did_they_contact_us", "signed_up_by", "tos_privacy_consent"]
  }];
  ...
  // in a half-width tile. 880px gave the tiles the ~50px each they were
  ...
  // Nudged to 900px for a little more room in every desktop row — each
  // half-width tile gains ~10px, nothing re-wraps narrower, and the page's
  // own content width still has room to spare (Amrit, 20 Sep 2026).
  var FORM_WIDTH_CAP = 900;
  function injectFormWidthStyles() {
    ...
    if (!document.getElementById("contour-form-width-styles")) {
      ...
      style.textContent =
        // The page's own stylesheet carries !important on some width rules,
        // which beats a plain rule from here however late it lands, so every
        // rule in this block is marked too (Amrit, 20 Sep 2026).
        ".container-form { max-width: " + FORM_WIDTH_CAP + "px !important; }" +
        ".hs-form { max-width: " + FORM_WIDTH_CAP + "px !important; }" +
  ...
  var CONTACT_FORMAT_FIELDS = [
    ...
    {
      selector: FIELD_SELECTORS.addGuardianFirstName,
      kind: "name",
      message: NAME_MESSAGE,
      errorClass: "contour-add-guardian-firstname-error"
    },
    {
      selector: FIELD_SELECTORS.addGuardianLastName,
      kind: "name",
      message: NAME_MESSAGE,
      errorClass: "contour-add-guardian-lastname-error"
    },
    {
      selector: FIELD_SELECTORS.addGuardianEmail,
      kind: "email",
      message: "Please enter a valid email address.",
      errorClass: "contour-add-guardian-email-error"
    }
  ];
  ...
  var EMAIL_VERIFY_SLOTS = [
    ...
    { key: "addGuardianEmail", selector: FIELD_SELECTORS.addGuardianEmail }
  ];
  ...
  // Only one panel is ever on the page, so a fixed id is safe and keeps the
  // trigger's aria-describedby pointing at something that exists.
  var STUDENT_RECOGNITION_TIP_ID = "contour-recognition-tip";
  // Said twice in the panel and shown once: the tooltip carries it for a
  // pointer, the block under the button carries it on a phone. Held here so
  // the two cannot drift apart.
  // Ends mid-sentence on purpose: the clear link is the last clause, not a
  // second line under it (Amrit, 20 Sep 2026). No name in here — the title
  // two lines up already asks "Is this you, <name>?", and a third of matched
  // records carry no firstname at all, so the pronoun is both shorter and the
  // only version that always works.
  var STUDENT_RECOGNITION_ESCAPE_TEXT = "Not you? Use a different email to sign up as someone new, or ";
  var STUDENT_RECOGNITION_CLEAR_LABEL = "clear the form";
  ...
  var STUDENT_RECOGNITION_GUARDIAN_TITLE = "We have this as a guardian's email in our records";
  // Both say the same thing first — we hold this on a grown-up's record — and
  // then differ only in the way out, because the way out differs. On the
  // Guardian flow the address is simply in the wrong one of two boxes. On the
  // Student flow the button below the message does the switch, so the words
  // stop short of instructions the button has already taken care of.
  var STUDENT_RECOGNITION_GUARDIAN_BODY_GUARDIAN_FLOW =
    "To continue, move it up to Guardian Email and give the student their own address here.";
  var STUDENT_RECOGNITION_GUARDIAN_BODY_STUDENT_FLOW =
    "To continue, sign up as the parent or guardian, or use the student's own email here.";
  ...
  /* Whether the visitor has tried to submit while one of these blocks was
     standing. Until they have, every verdict is presented the same neutral way
     the student offer always was: a grey panel, no red on the box. The
     guardian verdict used to go red the moment the field blurred, which made
     two states that block submission for the same reason look like different
     severities (Amrit, 20 Sep 2026).

     Pressing submit is what turns both of them red, panel and box together,
     because that is the moment the block actually costs the visitor something.
     Keyed to the address, so correcting it drops the alarm rather than
     carrying it onto the next thing they type. */
  var studentRecognitionRaisedFor = "";
  function studentRecognitionIsRaised(value) {
    return !!value && studentRecognitionRaisedFor === value;
  }
  ...
  // Only a clear address leaves. A match on either kind of record is a block,
  // and so is a check that has not come back — the whole point of the check is
  // that a second account cannot be opened behind its back, and a check that
  // silently passes when the network is down is not a check (Amrit, 19 Sep
  // 2026). Not-yet-asked still passes here; the pending gate resolves that one
  // before the form leaves.
  //
  // The cost of this is real and worth saying out loud: while the prefetch
  // endpoint is down, nobody can submit this form. The unknown panel's "Try
  // again" is the only way through, by design.
  function studentRecognitionIsValid() {
    ...
    if (verdict.status === "clear") return true;
    // "unknown" is our failure, not theirs — see blockOnUnresolvedEmailCheck.
    return verdict.status === "unknown" && !featureEnabled("blockOnUnresolvedEmailCheck");
  }
  ...
  function injectStudentRecognitionStyles() {
    ...
    style.textContent = "" +
      ...
      //
      // It runs the full width of the section's content area — no side margins
      // of its own. It used to sit 24px in on both sides to line its edges up
      // with the field boxes, which left a gutter down each side of a panel
      // that is already inside the section box's own 22px. Its edges now land
      // where the card's edges land, and the 24px moves into its padding, so
      // the text still starts on the same grid line as the labels above it.
      //
      // Four rows, two columns. The footer spans. The title, body and failure
      // note share their rows with the button, which spans those three and
      // centres against them, so the button never owns a row of its own —
      // that is what keeps the panel wide and shallow.
      ".hs-form " + c + " { flex: 0 0 100%; width: 100%; order: 99; box-sizing: border-box; margin: 6px 0 0; padding: 14px 24px 16px; background: rgba(12,49,102,0.045); border: 1px solid rgba(12,49,102,0.12); border-radius: 16px; flex-basis: 100%;" +
      " display: grid; grid-template-columns: auto minmax(0, 1fr) auto; grid-template-areas: \'badge title cta\' \'badge body cta\' \'badge note cta\'; column-gap: 24px; align-items: start; }" +
      // The badge column owns its own gutter, narrower than the 24px between
      // the prose and the button — it belongs to the title it sits beside.
      // Pinned to the title by default. Centring it down the whole panel was
      // tried and reverted (Amrit, 20 Sep 2026): against three lines of prose
      // it lands beside the middle of a sentence and points at nothing.
      //
      // The exception is a panel only two lines deep, where the midline and
      // the title are near enough the same place and centring reads better.
      // That cannot be asked in CSS, so JS measures the body and sets
      // --compact. See syncStudentRecognitionBadgeAlignment.
      ".hs-form " + c + "__badge { grid-area: badge; align-self: start; margin-right: -10px; flex: 0 0 auto; display: flex; align-items: center; justify-content: center; width: 32px; height: 32px; box-sizing: border-box; border: 2px solid " + navy + "; border-radius: 50%; background: #007AFF; color: #FFFFFF; }" +
      // The disc stays the form's blue whoever the record belongs to (Amrit,
      // 20 Sep 2026): the panel's own red tint and border already carry the
      // severity, and a red disc inside a red panel doubled up on it. Only a
      // check that could not run goes grey, because grey is the absence of an
      // answer rather than a worse one.
      ".hs-form " + c + "__badge--muted { background: #8595AE; }" +
      ".hs-form " + c + "--compact " + c + "__badge { align-self: center; }" +
      ...
      // Stretched across the three rows it spans rather than centred in them,
      // so the rule down its left side runs the full height of the text it
      // divides; the button is then centred inside it by the flex box. The
      // rule is the panel's own hairline — it splits the panel into what it
      // has to say and what it wants you to do (Amrit, 18 Sep 2026).
      ".hs-form " + c + "__cta { grid-area: cta; position: relative; align-self: stretch; justify-self: end; display: flex; align-items: center; padding-left: 24px; border-left: 1px solid rgba(12,49,102,0.12); }" +
      /* The cell is not the region. It spans the three grid rows and ends at
         the grid's content edge, so three strips of the right-hand side sat
         outside it and still raised the tooltip: the panel's 14px top padding,
         its 16px bottom padding, and the 24px of right padding beyond the
         button. The 24px grid gap on the left was outside it too.

         So the hit area is pushed out to the panel's own border on all four
         sides, in the panel's padding values. A pseudo-element rather than a
         negative margin, which would drag the separator rule out with it. */
      '.hs-form ' + c + '__cta::before { content: ""; position: absolute; top: -14px; right: -24px; bottom: -16px; left: -24px; }' +
      // The pseudo-element is positioned and the button was not, so it painted
      // over the button and swallowed its clicks. Positioning the button puts
      // it back on top: both are z-index auto, so they paint in DOM order and
      // the button comes second.
      ".hs-form button" + c + "__send { position: relative; }" +
      ...
      ".hs-form " + c + "--muted { border-style: dashed; }" +
      ...
      //
      // A standing label ("Existing sign-up") sat above this for one pass and
      // came out again: it spent a whole line, and the gap under it, saying
      // what the question underneath already says (Amrit, 18 Sep 2026).
      ".hs-form " + c + "__title { display: block; margin: 0 0 6px; font-size: 15.5px; font-weight: 600; line-height: 1.35; color: " + navy + "; }" +
      // Half a word-space between the name and the question mark, so the "?"
      // reads as the sentence's punctuation rather than as part of the name
      // (Amrit, 19 Sep 2026). Set here rather than as a space character in
      // the text: a real space would be a line-break opportunity, and the
      // question mark could end up alone on the next line.
      ".hs-form " + c + "__name { font-weight: 800; margin-right: 2px; }" +
      ...
      // Both alternatives to the lime: this panel only ever offers one action,
      // and the highlighter is reserved for the one that finishes the job.
      // Switching flow and retrying a check are both navigation.
      ".hs-form button" + c + "__send--switch { background: " + navy + "; color: #FFFFFF; transition: background-color .15s ease, transform .15s ease, box-shadow .15s ease; }" +
      // filter: brightness on a dark fill barely moves, so this one lifts and
      // lightens instead of leaning on the shared hover.
      ".hs-form button" + c + "__send--switch:hover { filter: none; background: #164C93; transform: translateY(-1px); box-shadow: 0 4px 12px rgba(12,49,102,0.22); }" +
      ".hs-form button" + c + "__send--switch:active { transform: translateY(0); box-shadow: 0 1px 4px rgba(12,49,102,0.20); }" +
      ".hs-form button" + c + "__send--switch:focus-visible { outline: none; box-shadow: 0 0 0 3px rgba(12,49,102,0.30); }" +
      ".hs-form button" + c + "__send--quiet { background: transparent; color: " + navy + "; box-shadow: inset 0 0 0 1px rgba(12,49,102,0.35); }" +
      ".hs-form button" + c + "__send--quiet:hover { filter: none; background: rgba(12,49,102,0.06); }" +
      ...
      // --- the "not your account?" tooltip -----------------------------------
      // A 16px ring on the end of the question, sized to sit inside the
      // title's line box rather than push it open.
      // Two more word-spaces of air: at 11px the glyph read as part of the
      // question mark rather than as its own control (Amrit, 18 Sep 2026).
      ".hs-form " + c + "__info { position: relative; display: inline-block; margin-left: 20px; top: 1px; }" +
      // The panel says this twice and shows it once. Off here, on inside the
      // phone media query, where the tooltip is the one that goes.
      ".hs-form " + c + "__hint { display: none; }" +
      ".hs-form button" + c + "__info-btn { appearance: none; -webkit-appearance: none; display: block; width: 14px; height: 14px; padding: 0; border: 0; border-radius: 50%; background: transparent; color: rgba(12,49,102,0.55); line-height: 0; cursor: pointer; transition: color .15s ease; }" +
      ".hs-form " + c + ":hover button" + c + "__info-btn, .hs-form " + c + "__info:hover button" + c + "__info-btn, .hs-form " + c + "__info.is-open button" + c + "__info-btn { color: " + navy + "; }" +
      ".hs-form button" + c + "__info-btn:focus-visible { outline: none; box-shadow: 0 0 0 3px rgba(12,49,102,0.25); }" +
      // Navy card with the highlighter on the link, the one pairing that puts
      // the lime on something legible (10.9:1, where lime as text on white is
      // 1.2:1) — the same trick the "You" badge uses.
      //
      // Anchored to the trigger's left edge rather than centred on it: the
      // name in front of it is whatever the CRM holds, so a centred tooltip
      // would drift with the name's length and could hang off the panel.
      ".hs-form " + c + "__tip { position: absolute; left: -10px; bottom: calc(100% + 9px); z-index: 5; box-sizing: border-box; width: max-content; max-width: 340px; padding: 12px 14px; background: " + navy + "; color: #FFFFFF; border-radius: 10px; font-size: 12.5px; font-weight: 400; line-height: 1.42; text-align: left; white-space: normal; opacity: 0; visibility: hidden; transform: translateY(3px); transition: opacity .13s ease, transform .13s ease, visibility .13s; }" +
      // The pointer, and the bridge that keeps the hover alive across the 9px
      // between the tooltip and the glyph it belongs to.
      '.hs-form ' + c + '__tip::after { content: ""; position: absolute; top: 100%; left: calc(17px - var(--tip-shift, 0px)); border: 6px solid transparent; border-top-color: ' + navy + '; }' +
      '.hs-form ' + c + '__tip::before { content: ""; position: absolute; top: 100%; left: 0; right: 0; height: 9px; }' +
      // Hovering the message side of the panel raises it, not just the glyph
      // (Amrit, 18 Sep 2026). Raised that way it stays inert, from the base
      // rule above: it sits over the Email box it is telling you to retype,
      // and a pointer crossing it on the way up there must not have its click
      // swallowed by a tooltip nobody asked for.
      //
      // This rule deliberately sets no pointer-events of its own. It did once,
      // and adding :not(:has()) to it later pushed it to (0,6,0) against the
      // glyph rule's (0,4,0) — so "none" outranked "auto" even while the glyph
      // was hovered, the tooltip went click-through, and moving onto it
      // dropped the hover and shut it before the clear link could be reached.
      // Stating the default once at the base and only ever raising it here is
      // what keeps the two rules out of each other's way.
      ".hs-form " + c + ":hover:not(:has(" + c + "__cta:hover)) " + c + "__tip { opacity: 1; visibility: visible; transform: none; }" +
      // No pointer-events rules anywhere on the tooltip any more, and that is
      // the fix, not an omission. It used to be inert whenever the panel
      // rather than the glyph had raised it, which meant the only way in was
      // to walk up through the 14px glyph: approach it from the side or from
      // above and the pointer fell straight through, the panel stopped being
      // hovered, and the thing vanished before it could be clicked (Amrit,
      // 20 Sep 2026).
      //
      // Visible now means reachable. `visibility: hidden` already keeps the
      // closed state from catching anything, and because the tooltip is a
      // descendant of the glyph's wrapper, the pointer landing on it from any
      // direction keeps that wrapper hovered and so keeps it open.
      ".hs-form " + c + "__info:hover " + c + "__tip, .hs-form " + c + "__info:focus-within " + c + "__tip, .hs-form " + c + "__info.is-open " + c + "__tip { opacity: 1; visibility: visible; transform: none; }" +
      // Lime, but plain: underlined it read as a marker stroke across the
      // tooltip rather than as a link. The rule comes back on hover so the
      // affordance is still there when it is being reached for.
      ".hs-form button" + c + "__clear { display: inline; vertical-align: baseline; margin: 0; padding: 0; appearance: none; -webkit-appearance: none; border: 0; background: transparent; color: " + lime + "; font: inherit; font-size: 12.5px; font-weight: 700; line-height: 1.3; text-align: left; text-decoration: none; cursor: pointer; }" +
      ".hs-form button" + c + "__clear:hover { text-decoration: underline; text-underline-offset: 2px; }" +
      ".hs-form button" + c + "__clear:focus-visible { outline: none; box-shadow: 0 0 0 3px rgba(215,252,61,0.45); border-radius: 4px; }" +
      ...
      " .hs-form " + c + " { margin: 4px 0 0; padding: 15px 20px 18px; flex-basis: 100%; }" +
      // Badge beside the title only. Giving it a column of its own the whole
      // way down would take 42px off a text column that has already been
      // tightened twice; spanning the prose under it costs nothing.
      " .hs-form " + c + " { grid-template-columns: auto minmax(0, 1fr); grid-template-areas: \'badge title\' \'body body\' \'cta cta\' \'note note\' \'hint hint\'; column-gap: 12px; }" +
      // Centred on the title rather than pinned to its top: the title wraps to
      // two lines on a phone, and a top-aligned disc beside a two-line heading
      // sat visibly high (Amrit, 20 Sep 2026).
      " .hs-form " + c + "__badge { width: 30px; height: 30px; margin-right: 0; align-self: center; }" +
      " .hs-form " + c + "__badge svg { width: 17px; height: 17px; }" +
      " .hs-form " + c + "__cta { justify-self: stretch; margin: 15px 0 0; padding-left: 0; border-left: 0; }" +
      " .hs-form button" + c + "__send { width: 100%; justify-content: center; }" +
      // The panel answers the Email box, so on a stacked card it follows the
      // Email box — under the Phone number it put a field the visitor may not
      // even need between the question and the thing it is about (Amrit,
      // 18 Sep 2026). Only the phone is moved: everything else keeps order 0
      // and so keeps DOM order. Both anchors are covered — the top-level
      // fieldset the Student flow lands on, and the group host the Guardian
      // flow lands on.
      " .hs-form fieldset.contour-person-card__row--guardian > .hs_phone { order: 3; }" +
      " .hs-form .contour-person-group-host > .hs_student_phone_number.contour-person-card__row { order: 3; }" +
      " .hs-form " + c + " { order: 2; }" +
      // Tighter set: the same sentence runs to four lines on a phone, so the
      // lines come in and the title comes down with them.
      // The type stays compact; the gaps between the blocks do not. Squeezing
      // both at once ran the question, the offer, the button and the escape
      // hatch together into one grey mass (Amrit, 18 Sep 2026).
      /* The badge and the title share a row here, and only the badge was
         centred in it — the title kept the container's align-items: start, so
         against a 30px disc a one-line title sat about 5px high and the two
         never lined up. Both centre now, at one line or two.

         The gap under the title moves off the title and onto the body. As a
         margin-bottom it was inside the badge's own row, so growing it pushed
         the row taller and dragged the centred badge down with it, which is
         the opposite of lining them up (Amrit, 20 Sep 2026). */
      " .hs-form " + c + "__title { font-size: 15px; margin-bottom: 0; align-self: center; }" +
      " .hs-form " + c + "__body { font-size: 13px; line-height: 1.42; margin-top: 12px; }" +
      // The tooltip goes: there is no hover to open it with, and opened above
      // the glyph it would cover the Email box it is telling you to retype.
      " .hs-form " + c + "__info { display: none; }" +
      " .hs-form " + c + "__hint { display: block; grid-area: hint; margin: 16px 0 0; font-size: 12.5px; line-height: 1.4; color: #6B7C99; }" +
      // Navy, not the tooltip's lime: on the panel's pale wash lime is 1.2:1
      // and unreadable — it only works knocked out of the navy card.
      " .hs-form button" + c + "__clear--inline { margin: 0; color: " + navy + "; text-decoration: underline; text-underline-offset: 2px; }" +
      " }" +
  ...
  // The "not your account?" escape hatch. It used to be a third line along the
  // bottom of the panel, which spent a row and a rule on the least likely
  // outcome; as a tooltip on the question itself it is one glyph until it is
  // wanted (Amrit, 18 Sep 2026).
  //
  // Opened by hover, by keyboard focus, and by click. Hover alone would hide
  // it from anyone on a touch screen and from anyone tabbing; the click
  // toggle is what makes the clear link inside it reachable at all on a
  // phone. The tooltip is a child of the trigger's wrapper, so moving the
  // pointer up into it keeps the wrapper hovered — and ::before bridges the
  // 9px between them so the hover does not drop out on the way.
  // Both halves of the escape hatch. Which one is on screen is a media query's
  // business, not this function's: the glyph and its tooltip for a pointer,
  // and a plain block under the button on a phone, where there is no hover to
  // open a tooltip with and it would cover the fields anyway (Amrit, 18 Sep
  // 2026). Both are built rather than one picked at build time — the panel is
  // only rebuilt when the verdict changes, so a layout chosen here would be
  // stuck the moment the phone was turned sideways.
  function studentRecognitionEscapeHatch(titleEl, panel, base) {
    studentRecognitionInfo(titleEl, base);
    var hint = document.createElement("span");
    hint.className = base + "__hint";
    hint.appendChild(document.createTextNode(STUDENT_RECOGNITION_ESCAPE_TEXT));
    var clear = document.createElement("button");
    clear.type = "button";
    clear.className = base + "__clear " + base + "__clear--inline";
    clear.textContent = STUDENT_RECOGNITION_CLEAR_LABEL;
    clear.addEventListener("click", function () {
      clearStudentRecognitionIdentity();
    });
  ...
  function studentRecognitionInfo(parent, base) {
    var wrap = document.createElement("span");
    wrap.className = base + "__info";
    var btn = document.createElement("button");
    btn.type = "button";
    btn.className = base + "__info-btn";
    btn.appendChild(studentRecognitionInfoIcon());
    btn.setAttribute("aria-label", "Not you?");
    btn.setAttribute("aria-expanded", "false");
    btn.setAttribute("aria-describedby", STUDENT_RECOGNITION_TIP_ID);
    var tip = document.createElement("span");
    tip.className = base + "__tip";
    tip.id = STUDENT_RECOGNITION_TIP_ID;
    tip.setAttribute("role", "tooltip");
    tip.appendChild(document.createTextNode(STUDENT_RECOGNITION_ESCAPE_TEXT));
    var clear = document.createElement("button");
    clear.type = "button";
    clear.className = base + "__clear";
    clear.textContent = STUDENT_RECOGNITION_CLEAR_LABEL;
    clear.addEventListener("click", function () {
      clearStudentRecognitionIdentity();
    });
    ...
    btn.addEventListener("click", function () {
      ...
      if (open) {
        positionStudentRecognitionTip(tip);
        // Tap anywhere else to dismiss. Bound only while it is open, and on
        // the next frame so the click that opened it does not close it again.
        setTimeout(function () {
    ...
    // Measured on the way in, for hover and for keyboard alike.
    wrap.addEventListener("mouseenter", function () { positionStudentRecognitionTip(tip); });
    wrap.addEventListener("focusin", function () { positionStudentRecognitionTip(tip); });
    wrap.appendChild(btn);
    wrap.appendChild(tip);
    parent.appendChild(wrap);
    // The panel opens it too, so the panel has to measure it too — entering
    // over the body text never crosses the glyph. parent is already in the
    // panel by the time this runs, so closest() finds it.
    var panel = parent.closest ? parent.closest("." + STUDENT_RECOGNITION_PANEL_CLASS) : null;
    if (panel) panel.addEventListener("mouseenter", function () { positionStudentRecognitionTip(tip); });
    return wrap;
  }
  // The tooltip hangs off a glyph that sits after a name of unknown length,
  // so on a narrow card it can start past the point where 260px still fits —
  // it ran off the right edge on a phone. Rather than a second, arrow-less
  // layout at a breakpoint, it is measured against the panel on the way in
  // and slid back inside; the arrow takes the same shift the other way so it
  // stays over the glyph it belongs to.
  //
  // margin-left rather than transform: the open/close transition owns
  // transform, and an inline one would kill the animation.
  function positionStudentRecognitionTip(tip) {
    var panel = studentRecognitionPanelEl;
    if (!panel || !tip.getBoundingClientRect) return;
    tip.style.marginLeft = "";
    tip.style.removeProperty("--tip-shift");
    // visibility: hidden still lays out, so this measures correctly while
    // the tooltip is closed.
    var pb = panel.getBoundingClientRect();
    var tb = tip.getBoundingClientRect();
    var gap = 10;
    var shift = 0;
    if (tb.right > pb.right - gap) shift = (pb.right - gap) - tb.right;
    if (tb.left + shift < pb.left + gap) shift = (pb.left + gap) - tb.left;
    shift = Math.round(shift);
    if (!shift) return;
    tip.style.marginLeft = shift + "px";
    tip.style.setProperty("--tip-shift", shift + "px");
  }
  // Wipes the contact details and the saved draft, then puts the cursor back
  // in the email box. Deliberately stops at the contact card: the subjects,
  // year level and campus below it were never the thing in dispute, and
  // throwing them away to correct an email address would be its own bug
  // report. clearDraft() goes too, or a reload would hand the old identity
  // straight back.
  function clearStudentRecognitionIdentity() {
    PERSON_GROUPS.forEach(function (group) {
      group.fields.forEach(function (config) {
        clearContactFieldValue(config.selector);
      });
    });
    clearDraft();
    var input = studentRecognitionInput();
  ...
  // The phone boxes are the reason this is not a one-liner: the visible box
  // is .contour-phone-proxy and the submitted box is the real input behind
  // it, and the proxy is the sole source — clearing only the named field
  // would leave the digits on screen.
  function clearContactFieldValue(selector) {
    var el = q(selector);
    if (!el) return;
    var wrap = fieldWrapper(el);
    var nodes = [el];
    if (wrap) {
      Array.prototype.forEach.call(wrap.querySelectorAll(".contour-phone-proxy"), function (node) {
  ...
  // The ring is drawn by the icon rather than by the button's border, so the
  // glyph and the circle around it stay one shape at 16px instead of a letter
  // rattling around inside a box.
  function studentRecognitionInfoIcon() {
    ...
    dot.setAttribute("cx", "8"); dot.setAttribute("cy", "4.85"); dot.setAttribute("r", "0.95");
    dot.setAttribute("fill", "currentColor");
    var stem = document.createElementNS(ns, "path");
    stem.setAttribute("d", "M8 7.3v4.3");
  ...
  // The same 32px disc the prefill banner wears at js/form1.js:4334, because
  // the two are the same kind of thing standing in the same place: a card that
  // says we already know something about you, with a title, a line of prose
  // and a way out. Deliberately NOT the banner's tick — a tick means done, and
  // this panel has done nothing yet; it is asking (Amrit, 19 Sep 2026). A
  // person mark says "we matched a record", which is what is actually true.
  // Neutral until the alarm is up, whatever the verdict. --warning is the
  // raised state now, not the guardian's permanent costume.
  /* One line of body or more than one. A two-line panel (title plus a body
     that fits on one line) puts its midline within a few pixels of the title,
     so the badge is centred there; anything taller and it goes back to the
     title, because the midline of a four-line panel is the middle of a
     sentence (Amrit, 20 Sep 2026).

     Width decides how the body wraps, so this re-runs on resize. The class is
     only written when it actually changes: the form's MutationObserver watches
     this subtree, and an unconditional write would wake it on every pass. */
  function syncStudentRecognitionBadgeAlignment() {
    var panel = studentRecognitionPanelEl;
    if (!panel || !panel.getBoundingClientRect) return;
    var body = panel.querySelector("." + STUDENT_RECOGNITION_PANEL_CLASS + "__body");
    var compact = false;
    if (body) {
      var cs = window.getComputedStyle ? window.getComputedStyle(body) : null;
      var lh = cs ? parseFloat(cs.lineHeight) : NaN;
      // line-height: normal reports as "normal"; 1.4 is close enough to pick
      // one line from two.
      if (!lh || isNaN(lh)) lh = (cs ? parseFloat(cs.fontSize) : 13.5) * 1.4;
      compact = body.getBoundingClientRect().height <= lh * 1.5;
    }
    var cls = STUDENT_RECOGNITION_PANEL_CLASS + "--compact";
    if (panel.classList.contains(cls) === compact) return;
    panel.classList.toggle(cls, compact);
  }
  var studentRecognitionResizeBound = false;
  function bindStudentRecognitionResize() {
    if (studentRecognitionResizeBound || typeof window.addEventListener !== "function") return;
    studentRecognitionResizeBound = true;
    var timer = null;
    window.addEventListener("resize", function () {
      if (timer) clearTimeout(timer);
      timer = setTimeout(syncStudentRecognitionBadgeAlignment, 120);
    });
  }
  function studentRecognitionPanelClass(variant, raised) {
    var cls = STUDENT_RECOGNITION_PANEL_CLASS;
    if (variant) cls += " " + STUDENT_RECOGNITION_PANEL_CLASS + "--" + variant;
    if (raised) cls += " " + STUDENT_RECOGNITION_PANEL_CLASS + "--warning";
    return cls;
  }
  function studentRecognitionBadge(panel, base, variant) {
    var badge = document.createElement("span");
    badge.className = base + "__badge" + (variant ? " " + base + "__badge--" + variant : "");
    badge.setAttribute("aria-hidden", "true");
    badge.appendChild(studentRecognitionBadgeIcon(
      variant === "muted" ? "search" : "person"));
    panel.appendChild(badge);
    return badge;
  }
  /* A person with a mark beside them, in the shapes Material Symbols uses:
     person_check when we found the visitor's own record, person_alert when we
     found a grown-up's, person_search when we could not look. Drawn here
     rather than pulled in as a font, in the same 1.9 stroke as the mail and
     info glyphs already in this panel, so the three sit together. The person
     is shifted left of centre to make room for the mark. */
  // Material Symbols "person_raised_hand", FILL1 (Apache 2.0), used as
  // shipped, on
  // the "That's me" button only. The discs all wear the same filled person:
  // the badge says whose record we found, and the colour and the panel around
  // it say what that means (Amrit, 20 Sep 2026).
  var STUDENT_RECOGNITION_HAND_PATH = "M80-40v-80h800v80H80Zm80-120v-240q-33-54-51-114.5T91-638q0-61 15.5-120T143-874q8-21 26-33.5t40-12.5q31 0 53 21t18 50l-11 91q-6 48 8.5 91t43.5 75.5q29 32.5 70 52t89 19.5q60 0 120.5 12.5T706-472q45 23 69.5 58.5T800-326v166H400v-37q0-34 23-58.5t57-24.5h160v-80H480q-67 0-113.5 48T320-197v37H160Zm207-447q-47-47-47-113t47-113q47-47 113-47t113 47q47 47 47 113t-47 113q-47 47-113 47t-113-47Z";
  function studentRecognitionBadgeIcon(kind) {
    var ns = "http://www.w3.org/2000/svg";
    if (kind === "hand") {
      var hand = document.createElementNS(ns, "svg");
      hand.setAttribute("viewBox", "0 -960 960 960");
      // 15, not the 17 the drawn glyphs take: Material's artwork runs to the
      // edge of its own box where mine leave a margin, so at a matched number
      // the hand sat visibly larger inside the same disc.
      hand.setAttribute("width", "15");
      hand.setAttribute("height", "15");
      hand.setAttribute("aria-hidden", "true");
      hand.setAttribute("focusable", "false");
      var hp = document.createElementNS(ns, "path");
      hp.setAttribute("d", STUDENT_RECOGNITION_HAND_PATH);
      hp.setAttribute("fill", "currentColor");
      hand.appendChild(hp);
      return hand;
    }
    var svg = document.createElementNS(ns, "svg");
    /* Centring these is arithmetic, not eyeballing, and the box is not the
       glyph. The filled person runs y 4.2 to 17.7, so its own middle is 10.95
       against a box whose middle is 10 — a whole unit low, which inside a
       32px disc is a visible drift. The person-and-mark shapes run y 2.35 to
       18.15 once their 1.9 stroke is counted, middle 10.25.

       Raising the viewBox's origin by that difference slides the artwork up
       by exactly the amount it was out, and leaves the box itself alone for
       the flex centring to handle (Amrit, 20 Sep 2026). Material's hand is
       already centred in its own box and takes no correction. */
    svg.setAttribute("viewBox", "0 " + (kind === "person" ? "0.95" : "0.25") + " 20 20");
    svg.setAttribute("width", "17");
    svg.setAttribute("height", "17");
    svg.setAttribute("aria-hidden", "true");
    svg.setAttribute("focusable", "false");
    ...
    // No mark beside them, so the person sits centred and solid: the disc is
    // already the container, and a filled silhouette inside it reads as one
    // shape at 17px where an outline plus a tick read as clutter (Amrit,
    // 20 Sep 2026).
    if (kind === "person") {
      var head = circle("10", "7.3", "3.1");
      head.setAttribute("fill", "currentColor");
      svg.appendChild(head);
      var body = path("M10 11.7c3.2 0 5.8 2.1 5.8 4.9 0 0.6-0.5 1.1-1.1 1.1H5.3c-0.6 0-1.1-0.5-1.1-1.1 0-2.8 2.6-4.9 5.8-4.9z");
      body.setAttribute("fill", "currentColor");
      svg.appendChild(body);
      return svg;
    }
    stroke(circle("7.7", "6.2", "2.9"));
    stroke(path("M2.4 16.3c0-2.9 2.4-4.7 5.3-4.7 0.7 0 1.4 0.1 2 0.3"));
    if (kind === "alert") {
      stroke(path("M15.4 9.7v3.5"));
      var dot = circle("15.4", "16.1", "1");
      dot.setAttribute("fill", "currentColor");
      svg.appendChild(dot);
    } else if (kind === "search") {
      stroke(circle("14.4", "13.4", "2.5"));
      stroke(path("M16.3 15.3l1.9 1.9"));
    } else {
      stroke(path("M12.2 14.1l1.9 1.9 3.6-3.8"));
    }
    return svg;
  }
  // The one-click version of "choose I'm a parent or guardian above". Only
  // ever offered on the Student flow — on the Guardian flow the switch has
  // already happened and the address is simply in the wrong box.
  function switchToGuardianContactType() {
    var radios = qAll(FIELD_SELECTORS.contactType);
    for (var i = 0; i < radios.length; i++) {
      if (radios[i].value === "Guardian" || radios[i].value === "Parent") {
        setCheckboxChecked(radios[i], true);
        return true;
      }
    }
    return false;
  ...
  function renderStudentRecognitionGuardianPanel(input) {
    var raised = studentRecognitionIsRaised(studentRecognitionValue(input));
    var key = "guardian|" + (isGuardianContactType() ? "g" : "s") + "|" + (raised ? "r" : "q");
    ...
    panel.textContent = "";
    panel.className = studentRecognitionPanelClass("", raised);
    panel.removeAttribute("role");
    // The panel itself must stay exposed now that it can hold a button: a
    // focusable control inside aria-hidden is reachable by tab and invisible
    // to the screen reader that lands on it. So the hiding moves down onto the
    // words instead, which are the only part the field's own error list is
    // already reading out.
    panel.removeAttribute("aria-hidden");
    var base = STUDENT_RECOGNITION_PANEL_CLASS;
    studentRecognitionBadge(panel, base, "warning");
    studentRecognitionTitle(panel, base, STUDENT_RECOGNITION_GUARDIAN_TITLE, "", "").setAttribute("aria-hidden", "true");
    studentRecognitionLine(panel, base + "__body", studentRecognitionGuardianBody()).setAttribute("aria-hidden", "true");
    // Student flow only. On the Guardian flow this button would switch the
    // form to the state it is already in, which is why the render key carries
    // the flow — the panel is rebuilt when it changes and the button goes.
    if (!isGuardianContactType()) {
      var actions = document.createElement("div");
      actions.className = base + "__cta";
      var swap = document.createElement("button");
      swap.type = "button";
      swap.className = base + "__send " + base + "__send--switch";
      // The badge's own glyph, deliberately this time: the button is the
      // answer to the badge, and repeating the mark ties them together
      // (Amrit, 20 Sep 2026).
      swap.appendChild(studentRecognitionBadgeIcon("hand"));
      swap.appendChild(document.createElement("span")).textContent = "That's me";
      swap.addEventListener("click", function () {
        if (!switchToGuardianContactType()) return;
        // setCheckboxChecked ticks the radio inside asProgrammaticEdit, and
        // onContactTypeChange ignores programmatic ticks on purpose — so the
        // refresh that normally follows a flow switch never runs, and the
        // block stayed up over a box that is no longer the student's. Stood
        // down here by hand, then again once HubSpot has swapped the
        // dependent group in and student_email actually exists.
        refreshStudentRecognition(false);
        syncFieldErrorAria();
        setTimeout(function () {
          refreshStudentRecognition(false);
  ...
  // The check could not be reached. It blocks, so it has to say so and it has
  // to offer a way forward — the stored "unknown" is what stops every later
  // pass from re-asking, so clearing it is what makes the retry a retry.
  function renderStudentRecognitionUnknownPanel(input, value) {
    var raised = studentRecognitionIsRaised(value);
    var key = "unknown|" + value + "|" + (studentRecognitionPending[value] ? "busy" : "idle") + "|" + (raised ? "r" : "q");
    if (studentRecognitionPanelIsCurrent(input, key)) return;
    var panel = studentRecognitionPanel(input);
    if (!panel) return;
    studentRecognitionRenderKey = key;
    panel.textContent = "";
    panel.className = studentRecognitionPanelClass("muted", raised);
    panel.removeAttribute("aria-hidden");
    panel.setAttribute("role", "status");
    var base = STUDENT_RECOGNITION_PANEL_CLASS;
    var busy = !!studentRecognitionPending[value];
    studentRecognitionBadge(panel, base, "muted");
    studentRecognitionTitle(panel, base, "We couldn't check that address", "", "");
    studentRecognitionLine(panel, base + "__body", featureEnabled("blockOnUnresolvedEmailCheck")
      ? "Our records didn't answer just now. Try again, you'll need this before you can submit."
      : "Our records didn't answer just now. You can try again, or carry on and we'll sort it out our end.");
    var actions = document.createElement("div");
    actions.className = base + "__cta";
    var retry = document.createElement("button");
    retry.type = "button";
    retry.className = base + "__send " + base + "__send--quiet";
    retry.textContent = busy ? "Checking\u2026" : "Try again";
    retry.disabled = busy;
    retry.addEventListener("click", function () {
      // Start the lookup before redrawing, not after: lookupStudentRecognition
      // sets the pending entry synchronously, and that entry is what the
      // render key reads to draw the disabled "Checking…" state. Clearing the
      // stored verdict first instead would take the panel off screen for the
      // length of the round trip.
      var request = lookupStudentRecognition(value, true);
      refreshStudentRecognition(false);
  ...
  function renderStudentRecognitionPanel(input, value, verdict) {
    var raised = studentRecognitionIsRaised(value);
    var key = ["student", value, studentRecognitionSendState[value] || "idle", verdict.canSendLink ? "1" : "0", verdict.fullName, raised ? "r" : "q"].join("|");
    if (studentRecognitionPanelIsCurrent(input, key)) return;
    var panel = studentRecognitionPanel(input);
    if (!panel) return;
    studentRecognitionRenderKey = key;
    ...
    if (sendState === "sent") {
      studentRecognitionBadge(panel, base, "sent");
      studentRecognitionEscapeHatch(studentRecognitionTitle(panel, base, "Check your inbox", "", ""), panel, base);
      // The address is the one thing worth re-reading in that sentence: it is
      // how someone catches a typo before they go hunting an inbox that will
      // never receive it. Built as nodes rather than markup because the line
      // helper sets textContent, and an address is user input.
      var sentLine = studentRecognitionLine(panel, base + "__body", "Your link is on its way to ");
      var sentAddress = document.createElement("strong");
      sentAddress.textContent = value;
      sentLine.appendChild(sentAddress);
      sentLine.appendChild(document.createTextNode(". Open it to carry on where you left off."));
      return;
    }

    var titleEl;
    studentRecognitionBadge(panel, base, "");
    if (verdict.fullName) {
      titleEl = studentRecognitionTitle(panel, base, "Is this you, ", verdict.fullName, "?");
    } else {
      titleEl = studentRecognitionTitle(panel, base, "We know this email", "", "");
    }
    studentRecognitionEscapeHatch(titleEl, panel, base);
    if (!verdict.canSendLink) {
      ...
      studentRecognitionLine(panel, base + "__body", "We've got your record here already, so this address can't start a new sign-up. Email hello@contoureducation.com.au and we'll sort it out.");
      return;
    }
    studentRecognitionLine(panel, base + "__body", "We already have your record saved with us. To add more subjects or update stored details, use the link in your email to continue with your pre-filled form.");
    ...
    buttonLabel.textContent = sendState === "sending" ? "Sending…" : "Email my link";
    ...
    if (sendState === "failed") {
      studentRecognitionLine(panel, base + "__note", "That didn't send. Try again, or email hello@contoureducation.com.au.");
    }
  }
  ...
  /* Raises the field's error list so the section card counts this box as
     unfinished and the alert announces once, with a choice about whether the
     box also turns red.

     Loud is for the guardian verdict, where the address really is in the wrong
     field and the red box is the point. Quiet is for the student verdict and
     for a check that could not run: both block the submit, but neither is the
     visitor having made a mistake, and the panel underneath is already saying
     the whole thing at full width. The list is clipped either way — see the
     note beside .contour-recognition-muted for why it cannot simply be
     hidden. */
  function markStudentRecognitionBlocked(input, message, loud) {
    var list = ensureContourError(input, STUDENT_RECOGNITION_ERROR_CLASS, message);
    var label = list && list.querySelector(".hs-error-msg");
    // Only when it differs: assigning textContent replaces the text node even
    // for an identical string, and that childList mutation wakes the form
    // observer, which runs this pass again, which writes it again.
    if (label && label.textContent !== message) label.textContent = message;
    if (list) list.classList.add("contour-recognition-muted");
    if (loud) {
      showContourError(input, STUDENT_RECOGNITION_ERROR_CLASS);
    } else {
      showErrorList(list);
    }
    return list;
  ...
  function refreshStudentRecognition(showErrors) {
    ...
    if (!verdict || verdict.status === "clear") {
      if (studentRecognitionRaisedFor === value) studentRecognitionRaisedFor = "";
      clearContourError(input, STUDENT_RECOGNITION_ERROR_CLASS);
      removeStudentRecognitionPanel();
      return;
    }
    if (verdict.status === "unknown") {
      // Quietly either way: a failed network call is not the visitor getting
      // something wrong, and a red box saying so would blame them for our
      // outage. Whether it also bars the submit is the flag's call; when it
      // does not, no error list is raised at all, so the card still counts as
      // finished and the summary stays silent.
      if (featureEnabled("blockOnUnresolvedEmailCheck")) {
        markStudentRecognitionBlocked(input, "We couldn't check this address. Try again before you submit.", studentRecognitionIsRaised(value));
      } else {
        clearContourError(input, STUDENT_RECOGNITION_ERROR_CLASS);
      }
      renderStudentRecognitionUnknownPanel(input, value);
      syncStudentRecognitionBadgeAlignment();
      return;
    }
    if (verdict.status === "guardian") {
      ...
      markStudentRecognitionBlocked(input, studentRecognitionGuardianMessage(), studentRecognitionIsRaised(value));
      renderStudentRecognitionGuardianPanel(input);
      syncStudentRecognitionBadgeAlignment();
      return;
    }
    // status === "student": a block now, not just an offer — the only way on
    // from here is the pre-fill link. Marked quietly for the same reason the
    // unknown case is: the panel beneath the box is full width and says the
    // whole thing, and a red box on top of it would read as "you typed your
    // own address wrong".
    markStudentRecognitionBlocked(input, "We already have your record. Use the link we email you.", studentRecognitionIsRaised(value));
  ...
  function studentRecognitionPendingGate(e) {
    ...
    lookupStudentRecognition(value, false).then(function () {
      ...
      var current = studentRecognitionInput();
      if (!current) return;
      // Same press, the verdict just landed late. It has to raise the alarm
      // itself: the validator's showError never ran for this attempt.
      studentRecognitionRaisedFor = studentRecognitionValue(current);
      refreshStudentRecognition(true);
    ...
    if (formRoot && !studentRecognitionGateBound) {
      ...
      registerSubmitValidator({
        ...
        showError: function () {
          var current = studentRecognitionInput();
          if (!current) return;
          // The one place the alarm goes up. Blur alone leaves it down.
          studentRecognitionRaisedFor = studentRecognitionValue(current);
          refreshStudentRecognition(true);
          reportFieldError(fieldWrapper(current) || current, current);
        },
  ...
  /* The built control is not the student's alone any more. The guardian boxes
     behind the "Add" prompt need the same country selector, and guardian_phone
     carries no useCountryCodeSelect either, so everything below is keyed on a
     config rather than on one selector — one entry per field that gets the
     built widget. A field whose HubSpot toggle IS switched on renders
     .hs-fieldtype-intl-phone and the builder bails, leaving the native widget
     alone; the state and error functions still read it correctly, because
     phoneGroupDial() knows both containers. */
  var INJECTED_PHONE_FIELDS = [{
    key: "student",
    selector: FIELD_SELECTORS.studentPhone,
    countryLabel: "Student phone country",
    errorClass: "contour-student-phone-error",
    boundAttr: "data-contour-student-phone-bound"
  }, {
    key: "addGuardian",
    selector: FIELD_SELECTORS.addGuardianPhone,
    // Optional in HubSpot, so a box holding nothing but its dial code is an
    // unanswered question rather than a half-finished one.
    optional: true,
    countryLabel: "Guardian phone country",
    errorClass: "contour-add-guardian-phone-error",
    boundAttr: "data-contour-add-guardian-phone-bound"
  }];
  function enhanceInjectedPhoneField(cfg) {
    var input = q(cfg.selector);
    if (!input || input.type === "hidden") return;
    ...
    select.setAttribute("aria-label", cfg.countryLabel);
    ...
    input.addEventListener("blur", function () {
      ...
      previousDial = studentPhoneDial(select);
      fireInputEvents(input);
      updateInjectedPhoneError(cfg, true);
    });
  }
  function enhanceStudentPhoneField() {
    INJECTED_PHONE_FIELDS.forEach(enhanceInjectedPhoneField);
  }
  function watchStudentPhoneField() {
    // The fields sit inside the Contact Type dependent group, so they aren't
    // in the DOM at init, and HubSpot re-renders them whenever native
    // validation fires — the same behaviour watchSchoolFieldRerender() handles.
    // enhanceInjectedPhoneField() is idempotent, so the mutations it makes
    // itself just no-op on the next observer callback.
    var observer = new MutationObserver(function () {
      INJECTED_PHONE_FIELDS.forEach(function (cfg) {
        enhanceInjectedPhoneField(cfg);
        bindInjectedPhoneProxy(cfg);
      });
    });
    observer.observe(formRoot, {
      childList: true,
  ...
  // The blur and input listeners enhanceInjectedPhoneField() puts on the named
  ...
  // synchronously, so injectedPhoneState() reads the settled value here.
  function bindInjectedPhoneProxy(cfg) {
    var input = q(cfg.selector);
    if (!input) return;
    var proxy = typableInput(input);
    if (!proxy || proxy === input) return;
    if (proxy.getAttribute(cfg.boundAttr) === "1") return;
    proxy.setAttribute(cfg.boundAttr, "1");
    proxy.addEventListener("input", function () {
      // Typing only ever takes the message away; it comes back on the way out.
      updateInjectedPhoneError(cfg);
    });
  ...
  function bindStudentPhoneProxy() {
    INJECTED_PHONE_FIELDS.forEach(bindInjectedPhoneProxy);
  }
  ...
  function injectedPhoneState(cfg) {
    var input = q(cfg.selector);
    if (!input) return "ok";
    var wrap = fieldWrapper(input);
    ...
    var select = group ? group.querySelector("select") : null;
    var dial = select ? studentPhoneDial(select) : phoneGroupDial(input);
    var national = dial && digits.indexOf(dial) === 0 ? digits.slice(dial.length) : digits;
    if (national === "") {
      // An optional box holding nothing but the dial code is an unanswered
      // box, not a half-answered one — the guardian block's fields are all
      // optional, so "incomplete" there would refuse a submission over a
      // question nobody was asked (Amrit, 22 Sep 2026).
      return cfg.optional ? "empty" : "incomplete";
    }
  ...
  function injectedPhoneIsValid(cfg) {
    var state = injectedPhoneState(cfg);
    return state === "ok" || state === "empty";
  }
  function studentPhoneState() {
    return injectedPhoneState(INJECTED_PHONE_FIELDS[0]);
  }
  function studentPhoneIsValid() {
    return injectedPhoneIsValid(INJECTED_PHONE_FIELDS[0]);
  }
  function ensureInjectedPhoneError(cfg) {
    var input = q(cfg.selector);
    if (!input) return null;
    var wrap = fieldWrapper(input) || input.parentElement;
    if (!wrap) return null;
    var existing = wrap.querySelector("." + cfg.errorClass);
    if (existing) return existing;
    var errorList = document.createElement("ul");
    errorList.className = "no-list hs-error-msgs inputs-list " + cfg.errorClass;
    errorList.setAttribute("role", "alert");
    hideErrorList(errorList);
    var errorItem = document.createElement("li");
    var errorLabel = document.createElement("label");
  ...
  function updateInjectedPhoneError(cfg, showWhenInvalid) {
    var input = q(cfg.selector);
    var errorList = ensureInjectedPhoneError(cfg);
    ...
    // A field left on HubSpot's own widget speaks for itself; two red lines
    // under one box saying much the same thing is worse than either alone.
    if (state === "ok" || state === "empty" || nativeErrorShowing(input)) {
      input.classList.remove("invalid", "error");
  ...
  function updateStudentPhoneError(showWhenInvalid) {
    updateInjectedPhoneError(INJECTED_PHONE_FIELDS[0], showWhenInvalid);
  }
  function enforceStudentPhoneValidation() {
    ...
    INJECTED_PHONE_FIELDS.forEach(function (cfg) {
      registerSubmitValidator({
        isValid: function () {
          return injectedPhoneIsValid(cfg);
        },
        showError: function () {
          updateInjectedPhoneError(cfg, true);
          var input = q(cfg.selector);
  ...
  function evaluateInternalOnlyFields() {
    ...
    // Last, and on every pass: the pass above clears the value on a fresh
    // render and re-hides the native select, so the identity has to be laid
    // back down afterwards or a HubSpot re-render silently unlocks the field
    // and strands the owner id on a name nobody chose.
    applyStaffGateIdentity();
  }
  ...
  /* =========================================================
     STAFF GATE IDENTITY
     -----------------------------------------------------------
     js/staff-gate.js signs a staff member in before this form is created,
     and hands over a verified identity plus an option -> owner-id directory.
     Two jobs here: put that identity into signed_up_by so attribution stops
     being self-reported, and carry the owner id in a hidden field so
     Workflow B can set the record owner without guessing from a name.

     The select is never disabled. A disabled control submits nothing, which
  ...
  /*
   * The option for staff who cannot find themselves. Its value is NA rather
   * than an owner id, and it is treated exactly as a public signup: no owner
   * is written, and Workflow B leaves the record's owner alone.
   */
  var STAFF_NO_OWNER_VALUE = "NA";

  // The option values ARE HubSpot owner ids now, so there is nothing to
  // resolve — except NA, which is not an owner at all.
  function ownerIdFromOption(value) {
    var picked = String(value === undefined || value === null ? "" : value).trim();
    return picked === STAFF_NO_OWNER_VALUE ? "" : picked;
  }
  var STAFF_UNMATCHED_TEXT = "We couldn't match your Contour account to this list — please pick your name.";

  function staffGateSession() {
    if (!window.ContourStaffGate || typeof window.ContourStaffGate.session !== "function") return null;
    return window.ContourStaffGate.session();
  }


  // Compared by value rather than by selector: option strings hold apostrophes
  // and nested brackets, and a selector built from one either throws or
  ...
  function writeOwnerId(value) {
    var hidden = q(FIELD_SELECTORS.signedUpByOwnerId);
    if (!hidden) return;
    hidden.value = value || "";
  }

  // Guarded inline rather than through a shared helper, because that is how
  // every other injector in this file does it.
  function injectStaffIdentityStyles() {
    if (document.getElementById("contour-staff-identity-styles")) return;
    var style = document.createElement("style");
    ...
    style.textContent = [
      ...
      "." + STAFF_HINT_CLASS + "{position:absolute;top:50%;right:22px;transform:translateY(-50%);",
      "width:20px;height:20px;padding:0;border:0;background:none;color:#8a94a6;cursor:help;",
      "display:flex;align-items:center;justify-content:center;z-index:3}",
      "." + STAFF_HINT_CLASS + " svg{width:20px;height:20px;display:block}",
      /*
       * !important because the Webflow page header sets input padding with
       * !important of its own, which beats an inline style however late it is
       * written — the trap that made an earlier inline padding-right silently
       * do nothing here. Without this the typed name runs under the icon.
       */
      "." + COMBOBOX_CLASS + " input." + STAFF_PAD_CLASS + "{padding-right:50px!important}",
      // Matched and locked: a badge, not a bare tick, so it reads as a state
      // the form is asserting rather than a mark someone left.
      "." + STAFF_HINT_CLASS + "[data-state='ok']{color:#0C3166;cursor:default;pointer-events:none}",
      // Red, because it discards something. The lift is small on purpose: it
      // has to read as "this responds" without drawing the eye away from the
      // field itself, which is where the person is actually working.
      "." + STAFF_HINT_CLASS + "[data-state='clear']{cursor:pointer;color:#d14343;",
      "transition:transform .15s ease,color .15s ease}",
      "." + STAFF_HINT_CLASS + "[data-state='clear']:hover,",
      "." + STAFF_HINT_CLASS + "[data-state='clear']:focus-visible{color:#b3261e;",
      // The base rule centres the icon with translateY(-50%), so the lift has
      // to carry that through or the icon jumps to the top of the field.
      "transform:translateY(calc(-50% - 2px))}",
      "." + STAFF_HINT_CLASS + ":hover,." + STAFF_HINT_CLASS + ":focus-visible{color:#0C3166}",
      "." + STAFF_HINT_CLASS + "__bubble{position:absolute;bottom:calc(100% + 10px);right:-6px;",
      "width:max-content;max-width:min(280px,70vw);padding:10px 12px;background:#0C3166;color:#fff;",
      "font-size:12.5px;line-height:1.45;font-weight:400;text-align:left;border-radius:8px;",
      "box-shadow:0 6px 20px rgba(12,49,102,.22);opacity:0;visibility:hidden;",
      "transition:opacity .15s ease,visibility .15s ease;pointer-events:none;z-index:4}",
      "." + STAFF_HINT_CLASS + "__bubble::after{content:'';position:absolute;top:100%;right:10px;",
      "border:6px solid transparent;border-top-color:#0C3166}",
      "." + STAFF_HINT_CLASS + ":hover ." + STAFF_HINT_CLASS + "__bubble,",
      "." + STAFF_HINT_CLASS + ":focus-visible ." + STAFF_HINT_CLASS + "__bubble{opacity:1;visibility:visible}"
    ].join("");
    document.head.appendChild(style);
  }

  /*
   * A hint inside the field rather than a line of error text beneath it.
   *
   * This is not a validation failure — the person is Contour staff, we simply
   * could not match them to a row in the map. A red sentence under the input
   * reads as "you did something wrong" for a state they cannot act on beyond
   * picking their own name, so it becomes a quiet icon on the input's right
   * edge that explains itself on hover or keyboard focus.
   */
  function staffHintIcon() {
    var svg = document.createElementNS("http://www.w3.org/2000/svg", "svg");
    svg.setAttribute("viewBox", "0 0 24 24");
    ...
    stem.setAttribute("x1", "12"); stem.setAttribute("y1", "11");
    stem.setAttribute("x2", "12"); stem.setAttribute("y2", "16");
    var dot = document.createElementNS("http://www.w3.org/2000/svg", "line");
    dot.setAttribute("x1", "12"); dot.setAttribute("y1", "7.75");
    dot.setAttribute("x2", "12"); dot.setAttribute("y2", "7.75");
  ...
  function staffTickIcon() {
    ...
    tick.setAttribute("points", "7.75 12.25 10.75 15.25 16.25 9.25");
  ...
  function staffClearIcon() {
    ...
    a.setAttribute("x1", "6.5"); a.setAttribute("y1", "6.5");
    a.setAttribute("x2", "17.5"); a.setAttribute("y2", "17.5");
    var b = document.createElementNS("http://www.w3.org/2000/svg", "line");
    b.setAttribute("x1", "17.5"); b.setAttribute("y1", "6.5");
    b.setAttribute("x2", "6.5"); b.setAttribute("y2", "17.5");
    svg.appendChild(a);
    svg.appendChild(b);
    return svg;
  }
  ...
  /*
   * Clearing a searchable dropdown is otherwise impossible: the <select> holds
   * the value, the combobox input only mirrors its label, and backspacing the
   * input leaves the underlying selection intact. So the only way back to an
   * empty field was a page reload.
   */
  function buildStaffClear(select) {
    var button = document.createElement("button");
    button.type = "button";
    button.className = STAFF_HINT_CLASS;
    button.setAttribute("aria-label", "Clear selection");
    button.appendChild(staffClearIcon());

    var bubble = document.createElement("span");
    bubble.className = STAFF_HINT_CLASS + "__bubble";
    bubble.setAttribute("role", "tooltip");
    bubble.textContent = "Clear selection";
    button.appendChild(bubble);
    button.addEventListener("click", function (event) {
      // Without this the click falls through and reopens the option list.
      event.preventDefault();
      event.stopPropagation();
      clearSelectValue(select);
      syncComboboxInput(select);
      applyStaffFieldIcon(select, false, STAFF_UNMATCHED_TEXT);
      var input = comboboxWrapperOf(select);
  ...
  function buildStaffTick() {
    var tick = document.createElement("span");
    tick.className = STAFF_HINT_CLASS;
    tick.appendChild(staffTickIcon());
    return tick;
  }

  function buildStaffHint(text) {
    var hint = document.createElement("button");
    hint.type = "button";
    hint.className = STAFF_HINT_CLASS;
    hint.setAttribute("aria-label", text);
    hint.appendChild(staffHintIcon());
    ...
    // A click would otherwise open the combobox list underneath it.
  ...
  /*
   * One icon on the field, in one of two states.
   *
   *   hint  an info mark, explaining on hover that we could not match this
   *         account — shown only while the field is still empty, because that
   *         is the only moment the explanation is actionable.
   *   ok    a navy tick in an outline circle, ONLY when we matched the account
   *         ourselves and locked the field. It is a badge for a state the form
   *         is asserting, so it carries no tooltip.
   *   clear a cross, when someone has picked their own name. It is the only
   *         way to empty a searchable dropdown — see buildStaffClear — and it
   *         deliberately does not claim the verification a tick would.
   *
   * The state is written to the element and compared before any swap — this
  ...
  function applyStaffFieldIcon(select, matched, text) {
    ...
    if (!anchor) return;
    var want = matched ? "ok" : select.value ? "clear" : "hint";
    var existing = anchor.querySelector("." + STAFF_HINT_CLASS);
    if (existing && existing.getAttribute("data-state") === want) return;
    if (existing && existing.parentNode) existing.parentNode.removeChild(existing);
    var field = anchor.querySelector("input");

    var icon = want === "ok" ? buildStaffTick()
      : want === "clear" ? buildStaffClear(select)
      : buildStaffHint(text);
    icon.setAttribute("data-state", want);
    // :has() is not universal, so the positioning context is set directly too.
    if (!anchor.style.position) anchor.style.position = "relative";
    // Keep the typed name clear of the icon.
    if (field) field.classList.add(STAFF_PAD_CLASS);
    anchor.appendChild(icon);
  }


  function lockStaffField(select) {
    select.setAttribute("aria-readonly", "true");
    select.classList.add(STAFF_LOCK_CLASS);
    /*
     * The combobox is the control a staff member actually sees: the native
     * <select> is put back to display:none by evaluateInternalOnlyField on
     * every observer pass. So lock the combobox's input rather than swapping
     * which element is visible — that swap is undone a tick later, leaving the
     * field with no visible control at all.
     */
    var wrapper = comboboxWrapperOf(select);
    if (!wrapper) return;
  ...
  // Per <select> NODE, not per field: a HubSpot re-render swaps in a new one,
  // and that new node needs its own listener while the old one is discarded
  // with it.
  var staffChangeBound = new WeakSet();

  function applyStaffGateIdentity() {
    if (!isInternalMode()) return;
    var select = q(FIELD_SELECTORS.signedUpBy);
    if (!select) return;

    var session = staffGateSession();
    if (!session) return;

    injectStaffIdentityStyles();

    // Resolve a manual pick through the same directory, so the hidden field
    // tracks whatever is actually selected. Bound once per node: this runs on
    // every observer pass, and an unguarded addEventListener would stack a
    // fresh copy each time.
    if (!staffChangeBound.has(select)) {
      staffChangeBound.add(select);
      select.addEventListener("change", function () {
        writeOwnerId(ownerIdFromOption(select.value));
        // Picking a name settles the field, clearing it opens the question
        // again — so the icon follows the selection, not just the match.
        applyStaffFieldIcon(select, false, STAFF_UNMATCHED_TEXT);
      });
    }

    /*
     * The gate hands over the signed-in person's HubSpot owner id, and the
     * dropdown's option values are owner ids, so the match is an exact string
     * comparison rather than anything parsed out of a label.
     *
     * An id with no matching option means they are not on the list yet — the
     * same outcome as HubSpot not knowing the address at all, so it falls
     * through to the manual pick rather than selecting nothing silently.
     */
    var ownerId = String(session.ownerId || "").trim();
    if (ownerId && selectHasOption(select, ownerId)) {
      if (select.value !== ownerId) {
        // Takes a SELECTOR, not an element — it resolves with q() internally.
        setSelectOrTextValue(FIELD_SELECTORS.signedUpBy, ownerId);
      }
      writeOwnerId(ownerId);
      lockStaffField(select);
      applyStaffFieldIcon(select, true, STAFF_UNMATCHED_TEXT);
      return;
    }

    /*
     * Derived from what is actually selected, never blanked outright. This
     * runs again on every pass, and by the second one a staff member may have
